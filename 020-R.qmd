...
![Life is a roller-coaster. You just have to ride it (Ronan Keating, 2000). Bildquelle:  [@horst_statistics_2024]. ](img/r_rollercoaster.png){#fig-roller width="80%"}
:::
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/020-R.html).
:::
...
### R & IDE (RStudio bzw. Positron)

Wenn wir sagen, "wir arbeiten mit R",
dann heißt das in unserem Fall, wir arbeiten mit R und mit einer *IDE* (integrierte Entwicklungsumgebung).
In diesem Buch stellen wir Ihnen zwei gleichwertige Optionen vor: *RStudio* und *Positron*, beide von der Firma Posit entwickelt.
Sie können frei wählen, welche der beiden IDEs Sie nutzen möchten -- alle Inhalte dieses Buchs funktionieren mit beiden.


:::: {.content-visible when-format="html"}


:::{#fig-rlove layout="[ 40, 20, 20, 20 ]"}
...
<!-- TODO: Screenshot/Logo Positron ergänzen -->
![](img/positron-logo-small.png){width=40%}



R und eine IDE wie RStudio oder Positron arbeiten gut zusammen.
:::
::::
...
Kurz gesagt: Das eigentlich Arbeiten besorgt R. Für den Komfort und die Übersicht ist die IDE zuständig. Auch eine Art von Arbeitsteilung! Das gilt genauso für Positron: R erledigt die Arbeit, die IDE (ob RStudio oder Positron) sorgt für Komfort.
...
::: {.content-visible when-format="html"}
Hier sehen Sie Screenshots von der Oberfläche von RStudio und Positron, s. @fig-rstudio.
...
<!-- TODO: Screenshot Positron ergänzen -->
![So sieht Positron aus](img/positron.png){#fig-positron width=50%}
:::
:::

## Installation von R und einer IDE {#sec-install-r}
...
Laden Sie es für Ihr Betriebssystem herunter unter <https://cloud.r-project.org>.
...
### Installation von RStudio oder Positron

RStudio und Positron sind beides *graphische Benutzeroberflächen* (graphical user interface, GUI) für R,
plus ein paar Goodies (in Form einer *integrierten Entwicklungsumgebung*, integrated development environment, IDE).
Beide werden vom gleichen Anbieter (Posit) entwickelt und sind kostenlos. Wählen Sie eine der beiden IDEs aus:
...
Laden Sie Positron für Ihr Betriebssystem (Windows, MacOS, Linux) herunter.^[<https://positron.posit.co/download.html>]
Wenn Sie die Installationsdatei heruntergeladen haben,
öffnen Sie diese Datei (Doppelklick) und Sie werden durch die Installation geführt.
(Sie benötigen u. U. Admin-Rechte auf Ihrem Computer.)
Positron ist neuer als RStudio und wird von Posit aktiv als gleichwertige Alternative weiterentwickelt; im Gegensatz zu RStudio unterstützt Positron neben R auch Python in derselben IDE.

<!-- TODO: Screenshot Positron-Installation ergänzen -->
:::
:::
...
:::{.callout-tip}
**Master-Shortcut** `Strg+Shift+P` öffnet die *Befehlspalette* -- damit finden und starten Sie *jeden* Befehl der IDE per Suchbegriff, ohne den passenden Klickpfad im Menü suchen zu müssen. Funktioniert identisch in RStudio und Positron. $\square$
:::
...
:::{.callout-note}
Für Positron gibt es aktuell keine offizielle Cloud-Version; nutzen Sie in diesem Fall entweder RStudio Cloud oder installieren Sie Positron lokal (Desktop). $\square$
:::
...
da Sie vermutlich keine Schreibrechte im Projektordner dieser netten Person (Ihrer Lehrkraft) haben.
...
Ihre IDE (RStudio oder Positron) starten, nicht R direkt. $\square$
:::



Wir verwenden beide Programme (R und eine IDE).
Aber wir *öffnen nur* die IDE.
RStudio bzw. Positron findet selbständig R und öffnet dieses "heimlich".
Öffnen Sie nicht noch extra R (sonst wäre R zweifach geöffnet).
Anstelle von *RStudio Desktop* (auf Ihrem Computer/Desktop) können Sie auch die *RStudio Cloud* (die Online-Version) starten -- für Positron gilt das nur lokal, s.o.
...
Man siehe zur Verdeutlichung Erweiterungen beim Browser Chrome, @fig-chrome.
...
In RStudio geht das Installieren komfortabel per Klick; in Positron nutzen Sie dafür meist die Konsole (Befehl `install.packages()`, s.u.), da Positron aktuell kein RStudio-identisches *Packages*-Panel mit Klick-Installation besitzt.
...
@fig-so-installieren verdeutlicht, wo Sie in RStudio klicken müssen, um Pakete zu installieren: Beim Reiter *Packages* auf *Install* klicken und dann den Namen des zu installierenden Pakets eingeben.
...
In Positron installieren Sie Pakete über die Konsole mit `install.packages()` (s.u.). Ein Klick-Assistent wie in RStudio ist aktuell nicht vorhanden.

<!-- TODO: Screenshot Positron-Paketinstallation ergänzen -->
:::
:::
...
>   [🧑‍🏫]{.content-visible when-format="html"}[\emoji{teacher}]{.content-visible when-format="pdf"} In RStudio können Sie im Reiter *Packages* nachschauen, welche Pakete
auf Ihrem Computer schon installiert sind; in Positron geben Sie dazu `installed.packages()` in der Konsole ein.


::: {.content-visible when-format="html"}
Diese Pakete brauchen Sie logischerweise dann *nicht* noch mal installieren, s. @fig-paket-installiert (RStudio); es sei denn, Sie wollen das Paket updaten.
...
Die einfachste Art, alle Pakete zu beenden, ist die Konsole bzw. Session neu zu starten: in RStudio über `Session > Restart R`, in Positron über den Neustart-Button in der Konsole (Symbol mit rundem Pfeil) bzw. den Befehl `Restart R Session` in der Befehlspalette (`Strg+Shift+P`).
...
Namen des Pakets in der Paketliste (Reiter *Packages*, in RStudio) sehen.
Ein bestimmtes R-Paket muss man nur *einmalig installieren*.
Aber man muss es *jedes Mal neu starten*,
wenn man R (bzw. die IDE) startet.
...
Ein *Projekt* (RStudio) bzw. ein *Workspace/Ordner* (Positron) ist letztlich ein Ordner,
der als "Basis" für eine Reihe von zusammengehörigen Dateien verwendet wird.
Sagen wir, Sie nennen Ihr Projekt `cool_stuff`.
Die IDE legt uns diesen Ordner an einem von uns gewählten Platz auf unserem Computer an.
...
Nutzen Sie Projekte/Workspaces, das macht Ihr Leben leichter -- praktischer, als das Arbeitsverzeichnis von Hand zu wählen oder mit Pfaden herumzubasteln.

::: {.content-visible when-format="html" unless-format="epub"}
:::{.panel-tabset}
### RStudio

RStudio-Projekte können Sie anlegen mit Klick auf das Icon, das einen Quader mit dem Buchstaben R darin anzeigt.
...
In Positron öffnen Sie stattdessen über `File > Open Folder…` den gewünschten Ordner als Workspace. Ein zu RStudio-Projekten identisches Konzept mit eigener `.Rproj`-Datei gibt es nicht; der geöffnete Ordner übernimmt die gleiche Funktion (Basisordner für relative Pfade).

<!-- TODO: Screenshot Positron-Workspace/Ordner öffnen ergänzen -->
:::
:::
...
vorgesehene Textdatei -- egal ob in RStudio oder Positron.
...
Einer ist: klicken Sie auf das Icon, das ein weißes Blatt mit einem grünen Pluszeichen zeigt, s. @fig-script-new.
...
Nutzen Sie `File > New File > R Script`, oder das Tastaturkürzel `Strg+N`. Der Menüpfad ist zu RStudio analog, das Icon-Set sieht in Positron aber anders aus.

<!-- TODO: Screenshot Positron neue Skriptdatei ergänzen -->
:::
:::

::: {.content-visible when-format="pdf"}
Einer ist: klicken Sie auf das Icon, das ein weißes Blatt mit einem grünen Pluszeichen zeigt, s. @fig-script-new (RStudio). In Positron nutzen Sie stattdessen `File > New File > R Script`.
...
::: {.content-visible when-format="epub"}
Einer ist: klicken Sie auf das Icon, das ein weißes Blatt mit einem grünen Pluszeichen zeigt (RStudio). In Positron nutzen Sie stattdessen `File > New File > R Script`.
:::
...
Quarto ist sowohl in RStudio als auch in Positron integriert.
Quarto ist eine komfortable und leistungsfähige Methode, um Dokumente mit R-Syntax anzureichern.
Sie sind aber nicht verpflichtet, Quarto zu nutzen. Stattdessen können Sie Ihre Syntax auch in Skriptdateien schreiben.


::: {.content-visible when-format="html"}
@fig-quarto zeit ein Beispiel für ein Quarto-Dokument (Screenshot aus RStudio; in Positron sieht die Bearbeitung sehr ähnlich aus).
...
Dann können Sie in RStudio oder Positron Quarto-Dateien erstellen.
Ein neues Quarto-Dokument können Sie in beiden IDEs erstellen mit Klick auf *File > New File > Quarto Document*.
...
Hält man sich aber nicht an die vorgegebene Reihenfolge,
...
- *Wo finde ich Hilfe zu einer bestimmten Funktion, z.$\,$B. `fun`?* Geben Sie dazu folgenden R-Befehl ein: `help(fun)`. Alternativ geben Sie den Namen der Funktion im Suchfeld beim Reiter `Help` ein (RStudio) bzw. im Help-Panel (Positron). Oder Googeln.
...
- *Ich muss mal grundlegend verstehen, wozu ein bestimmten R-Paket gut ist. Was tun?* Lesen Sie die Dokumentation ("Vignette") eines R-Pakets durch. Für das Paket `dplyr` bekommen Sie so einen Überblick über die verfügbaren Vignetten diese Pakets: `vignette(package = "dplyr")`. Dann suchen Sie sich aus der angezeigten Liste eine Vignette raus; mit `vignette("rowwise")` können Sie sich dann die gewünschte Vignette (z.$\,$B. `rowwise`) anzeigen lassen.
...
- *Irgendwie reagiert R komisch, vielleicht hat es sich aufgehängt?* Starten Sie R neu. Klicken Sie auf *Session > Restart R* (RStudio) bzw. auf den Neustart-Button in der Konsole (Positron).
...
Sie können auch von Ihrem Computer aus Daten in Ihre IDE importieren.
...
:::{.callout-note}
Diesen komfortablen Klick-Import-Assistenten bietet aktuell nur RStudio; in Positron gibt es kein 1:1-Äquivalent. Importieren Sie Daten in Positron stattdessen über R-Befehle wie `read.csv()` (s.o.) -- das funktioniert identisch in beiden IDEs. $\square$
:::
...
Ihre Kommilitonin Sarah behauptet:

> „Man braucht doch keine IDE! Reicht doch nur mit R. Ich schreibe meinen Code einfach in einem normalen Texteditor und führe ihn über die Kommandozeile mit `Rscript` aus – wozu der ganze Schnickschnack mit RStudio oder Positron?“

Entwickeln Sie ein Gegenargument: Zeigen Sie an konkreten Beispielen, warum eine IDE (wie RStudio oder Positron) einen echten Mehrwert gegenüber der reinen R-Installation bietet – auch wenn R technisch tatsächlich ohne IDE lauffähig ist.

### Lösung

Sarahs Aussage vermischt zwei verschiedene Dinge: *R* ist die Sprache bzw. der Interpreter, der Code ausführt – eine *IDE* (Integrated Development Environment) ist die Werkbank drumherum, die das Arbeiten mit dieser Sprache erleichtert. Technisch hat sie recht: R lässt sich in jedem Texteditor schreiben und per `Rscript datei.R` von der Kommandozeile ausführen. Trotzdem übersieht das Argument, was dabei verloren geht.

Erstens: Ohne IDE fehlt die *Sichtbarkeit* auf den aktuellen Programmzustand. Das *Environment*-Fenster einer IDE zeigt auf einen Blick, welche Objekte gerade existieren, welche Werte sie haben und welchen Typ (s. @sec-rvars). Ohne dieses Fenster muss man ständig `print()` oder `str()` aufrufen, um sich zu orientieren – gerade für Einsteigerinnen ein erheblicher Mehraufwand und eine häufige Fehlerquelle.

Zweitens: Eine IDE unterstützt aktiv beim *fehlerfreien Schreiben* von Code – Syntax-Highlighting macht Tippfehler sofort sichtbar (z.$\,$B. eine vergessene schließende Klammer), Autovervollständigung schlägt Funktions- und Variablennamen vor und verhindert Tippfehler bei Objektnamen, und die eingebaute Klammer- und Anführungszeichen-Prüfung verhindert einen großen Teil der Anfängerfehler von vornherein.

Drittens: Reproduzierbarkeit und Projektorganisation (s. @def-repro) sind ohne IDE erheblich mühsamer. RStudio- bzw. Positron-Projekte bündeln Arbeitsverzeichnis, Skripte und Daten an einem Ort und sorgen dafür, dass relative Pfade zuverlässig funktionieren – unabhängig davon, wer das Projekt auf welchem Rechner öffnet. Ohne diese Struktur hantieren Nutzer typischerweise mit absoluten Pfaden (`setwd("/Users/.../meineDaten")`), was Skripte auf anderen Rechnern sofort unbrauchbar macht.

Viertens: Praktische Werkzeuge wie das *Help*-Panel (sofortiger Zugriff auf Dokumentation ohne Kontextwechsel), das *Packages*-Panel (Pakete installieren/aktualisieren per Klick, s. @sec-r-pckgs) und die integrierte *Plots*-Anzeige (Grafiken erscheinen direkt neben dem Code, nicht in einem separaten, oft verdeckten Fenster) sparen im Alltag viel Zeit – gerade beim explorativen Arbeiten, wo man Code, Ausgabe und Grafik ständig im Blick haben muss.

Das Gegenargument "aber manche Profis nutzen nur einen minimalen Editor plus Kommandozeile" trifft zwar zu, geht aber am Kern vorbei: Diese Profis verzichten selten wirklich auf *alle* IDE-Funktionen – sie nutzen stattdessen meist leichtgewichtige Editoren mit eingebauten R-Erweiterungen (Syntax-Highlighting, Autovervollständigung, Konsolen-Integration), die dieselben Grundfunktionen bieten, nur in anderer Verpackung. Der Bedarf an diesen Funktionen verschwindet also nicht, er wird nur anders erfüllt. Für Einsteigerinnen und Einsteiger, die noch keine eigene, eingespielte Werkzeugkette haben, ist eine vollwertige IDE wie RStudio oder Positron daher der mit Abstand einfachste und fehlerärmste Einstieg. $\square$
:::
::::

::::{#exr-csv-xlsx-argument}
:::{.panel-tabset}
### Aufgabe

Ihr Kommilitone Jonas behauptet:

> „CSV ist doch einfach immer das bessere Format. XLSX braucht man in der Datenanalyse eigentlich nie – wer noch mit Excel-Dateien arbeitet, hat's einfach nicht verstanden.“

Entwickeln Sie ein differenziertes Gegenargument: Zeigen Sie, warum CSV zwar für die Datenanalyse meist die bessere Wahl ist, XLSX (bzw. Excel-Dateien allgemein) aber durchaus auch seine berechtigten Einsatzzwecke hat – Jonas' Pauschalurteil also zu kurz greift.

### Lösung

Jonas hat im Kern recht, aber sein Urteil ist zu pauschal. Für die *Datenanalyse selbst* – also das Einlesen, Verarbeiten und Weiterverwenden von Rohdaten in R – ist CSV in der Regel tatsächlich die bessere Wahl, aus mehreren Gründen: Eine CSV-Datei ist eine reine Textdatei, offen dokumentiert und ohne proprietäres Format – sie lässt sich mit jedem Texteditor öffnen, mit jeder Programmiersprache lesen und braucht keine Excel-Lizenz. Genau diese Transparenz und Einfachheit macht CSV robust gegenüber Versionswechseln von Software und ist ein wichtiger Baustein für Reproduzierbarkeit (s. @def-repro): Ändert sich zehn Jahre später das Excel-Dateiformat, bleibt eine CSV-Datei trotzdem lesbar. Außerdem lässt sich eine CSV-Datei sauber mit Versionskontrollsystemen wie Git verfolgen (Zeile-für-Zeile-Unterschiede), während sich Änderungen in einer binären XLSX-Datei kaum sinnvoll nachverfolgen lassen.

Trotzdem hat XLSX (bzw. das allgemeinere Excel-Format) legitime Stärken, die CSV *nicht* bietet, und die Jonas' "eigentlich nie"-Aussage widerlegen:

1. **Mehrere Tabellenblätter in einer Datei**: Eine XLSX-Datei kann mehrere zusammengehörige Tabellen (z.$\,$B. Rohdaten, Data-Dictionary, Auswertung) in einer einzigen Datei bündeln. Eine CSV-Datei repräsentiert dagegen immer nur *eine* Tabelle (s. @def-tidy) – für mehrere zusammengehörige Tabellen bräuchte man mehrere CSV-Dateien.
2. **Formatierung, Formeln und Kommentare**: Wenn eine Excel-Datei nicht als Rohdatenquelle für R, sondern als *Kommunikationsmittel* dient – etwa ein Bericht für Personen, die kein R nutzen –, sind bedingte Formatierung, eingebettete Formeln, Diagramme und Zellkommentare oft genau das, was gebraucht wird. Das leistet CSV grundsätzlich nicht.
3. **Eindeutigkeit von Datentypen**: In einer XLSX-Datei ist z.$\,$B. eine Datumsspalte als Datum gespeichert und eindeutig als solche erkennbar. Eine CSV-Datei speichert dagegen nur Text; ob `"01.02.2024"` ein Datum, eine Postleitzahl oder einfach eine Zeichenkette ist, muss beim Einlesen erst interpretiert werden – eine häufige Fehlerquelle.
4. **Verbreitung bei Nicht-Programmierenden**: In vielen Organisationen ist Excel das gemeinsame Werkzeug zwischen technischen und nicht-technischen Kolleginnen und Kollegen (s. @sec-import-mariokart zum Einlesen solcher Dateien in R). Daten von einer Fachabteilung kommen deshalb oft schlicht als XLSX an, nicht weil die Absender "es nicht verstanden haben", sondern weil das in ihrem Arbeitskontext das praktikable Format ist.

CSV hat im Gegenzug auch eigene Tücken, die man kennen sollte: Je nach Ländereinstellung wird als Trennzeichen mal ein Komma, mal ein Semikolon verwendet, und auch das Dezimaltrennzeichen (Punkt vs. Komma) unterscheidet sich – eine CSV-Datei ist also keineswegs automatisch "eindeutig", nur weil sie einfach aussieht.

Kurz: CSV ist der bessere *Standard* für die eigentliche Datenanalyse, weil es einfach, transparent, versionierbar und werkzeugunabhängig ist. XLSX ist aber kein "falsches" Format, sondern für andere Zwecke – mehrere verknüpfte Tabellenblätter, formatierte Berichte, Kommunikation mit Nicht-Programmierenden – oft die passendere Wahl. Die richtige Frage ist also nicht "CSV oder XLSX?", sondern "Wofür wird die Datei gebraucht?". $\square$
:::
::::
...
Alternativ können Sie in RStudio auch Excel-Dateien *ohne* R-Code importieren (der Import-Assistent ist, wie oben erwähnt, aktuell RStudio-exklusiv).
...
Irgendwann werden wir mit unserer Version von R und unserer IDE veraltet sein.
Installieren Sie dann einfach die neue Version von R und Ihrer IDE (RStudio oder Positron) wie oben beschrieben, s. @sec-install-r.

So updaten Sie Ihre R-Pakete: Klicken Sie im Reiter `Packages` auf `Update` (RStudio); in Positron aktualisieren Sie Pakete stattdessen per Konsolenbefehl `update.packages()`.
...
Denken Sie daran, dass Sie die Software (R, IDE, R-Paket),
