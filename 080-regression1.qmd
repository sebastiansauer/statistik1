...
Ihre Antwort lautet: 
"Im Schnitt haben die Studis bei der letzten Klausur ungefähr `r mean(round(noten2$y))`  der Punkte erzielt. 
...
ggplot(noten2) +
  ...
  annotate("label", x = -Inf, y = mean(noten2$y), 
           label = paste0("MW: ", round(mean(noten2$y))), hjust = 0) +
  theme_minimal()
```
...
#| label: fig-noten4
#| echo: false
#| fig-cap: "Noten und Lernzeit: Rohdaten (a) und mit Modell (b). Mittelwerte sind mit gestrichelten Linien eingezeichnet. Tonis Vorhersage ist mit einem Punkt gekennzeichnet."
#| layout-ncol: 2
#| fig-subcap: 
#|   - "Streudiagramm"
...
Wert für $\color{ycol}{y}$ handelt, nicht das tatsächliche (empirische, beobachtete) $\color{ycol}{y}$. 
...
als lineare Funktion von einer (oder mehreren) unabhängigen Variablen, $\color{xcol}{x}$, an, plus einem Fehlerterm, $\color{errorcol}{e}$ bzw. $\color{errorcol}{\epsilon}$, s. @eq-linear-model. $\square$
:::
...
Die Regressionskoeffizienten (hier synonym: Modellparameter) $\bat_0$ und $\beta_1$ wählt man so, dass die *Residuen* *minimal* sind.
...
dürfen Unterschiede in $r$ nicht gleich interpretiert werden wie Unterschiede in $R^2$. 
...
Mariokart-Daten laden, am besten ohne Extremwerte, s. @ lst-no-extreme und los geht's (und die üblichen Pakete starten, nicht vergessen)!
