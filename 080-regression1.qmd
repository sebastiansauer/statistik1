...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/080-regression1.html) (oder als [PDF](slides/080-regression1.pdf)).
:::
...
>    Der vorhergesagte Gesamtpreis eines Spiels liegt bei `r round(coef(lm3)[1])` Dollar "Sockelbetrag" plus `r round(coef(lm3)[2])` mal die Versandkosten.
...
estimate_expectation(lm3) |> 
  head(show_data = TRUE) |> print_md()  # nur die ersten paar vorhergesagten Werte
```
...
#| out-width: 75%
estimate_prediction(lm3, by = "ship_pr") |> 
  plot(show_data = TRUE)
```
