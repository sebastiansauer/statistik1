...
#| lst-cap: "Pfad zum Datensatz 'mariokart'"
#| lst-label: lst-mario-path
...
![**Was er NICHT sagt:** "Allerdings streuten die Werte der Gewichtsveränderung um 10kg um den Mittelwert herum."](img/teacher.png){width="25%"}
...
Die vertikalen grauen Balken in @fig-streuung kennzeichnen den (absoluten) Abstand von jeweils einem Datenpunkt zum Mittelwert (horizontale orange Linie). Je länger die vertikalen 'Abstandsbalken' insgesamt, desto größer die Streuung."
...
Im Beispiel von Marikoart: Da die Anzahl der Lenkräder mit dem Verkaufsgebot zusammenhängt, könnte es vielleicht sein, dass wir die Lenkräder-Anzahl da irgendwie nutzen könnten.
...
#| layout-ncol: 2
#| fig-subcap:
#|   - "Fehlerbalken im einfachen Modell: Ein Mittelwert; viel Streuung insgesamt. y ~ 1"
#|   - "Fehlerbalken im komplexen Modell: Zwei Mittelwerte; weniger Streuung in jeder Gruppe. Das erkennt man daran, dass die vertikalen, grauen Abstandsbalken im Schnitt kürzer sind als im einfachen Modell (links). y ~ G"
...
m %>% 
  ...
  geom_label(data = m_sum,
    aes(label = paste0("MW bei ", wheels, " Räder"), y = total_pr, color = factor(wheels)), x = max(m$ID), hjust = 1, size = 6) +
...
:::{#def-range}
Ein einfaches Streuungsmaß ist der *Range* $R$, 
definiert als Abstand von größtem und kleinsten Wert eines Merkmals $X: R = X_{max} - X_{min}. \square$
:::

:::{#exm-range}
Angenommen, wir haben einen Datensatz zum Merkmal "Alter" mit den Werte 1, 23, 42, 100. 
Dann beträgt der Range: $R = 100 - 1 = 99$. 
Das bedeutet, dass die Werte des Merkmals über 99 Einheiten (Jahre in diesem Fall) 
...
den wir mit $\bar{e}$ bezeichnen könnten.
...
$${\displaystyle \mathrm {MAE} ={\frac {\sum _{i=1}^{n}\left|y_{i}-\bar{y}\right|}{n}}={\frac {\sum _{i=1}^{n}\left|e_{i}\right|}{n}}=\bar{e}.}$$ {#eq-mae}
...
:::{#exm-iqr}
### IQR im Hörsaal
In einem Statistikkurs betragen die Quartile der Körpergröße: Q1: 1.65m, Q2 (Median): 1,70m, Q3: 1.75m. 
Der IQR beträgt dann: $IQR = Q_3-Q_1 = 1.75m - 1.65m = 0.10m$, d.h. 10 cm.$\square$
:::
...
Man kann zeigen, dass eine Normalverteilung sich komplett über ihren *Mittelwert* sowie ihre *Standardabweichung* beschreiben lässt [@lyon_why_2014].
Außerdem gilt: Sind Ihre Daten normalverteilt, 
dann sind die Abweichungen vom Mittelwert auch normalverteilt.
Denn wenn man eine Konstante zu einer Verteilung addiert (bzw. subtrahiert), "verschiebt man den Berg" ja nur zur Seite, 
ohne seine Form zu verändern, s. @fig-norm-dev.
...
![Illustration zur Varianz als "mittlerer Quadratfehler" [@cmglee_english_2015]](img/Variance_visualisation.svg.png){#fig-var}
...
Dazu berechnen Sie die Streuung in den Verkaufspreisen, s. @lst-mario-streu. $\square$
:::
...
Berechnung der Streuung des Verkaufpreises als Indikatoren für die Modellgüte des Mittelwerts. 
...
Statistiken sind ja schön ... aber Bilder sind auch gut, s. @fig-var. Datendiagramme eignen sich gut, um (grob) die Streuung einer Variable zu erfassen.
...
Als Formel ausgedrückt, lautet die Definition der Varianz einer Stichprobe wie folgt, s. @eq-var (hier geht es um die sog. unkorrigierte Stichprobenvarianz; um anhand einer Stichprobe die Varianz der zugehörigen Population zu schätzen, teilt man nicht durch $N$, sondern durch $N-1$) .
...
:::{#def-var}
### Varianz
Die Varianz ($s^2, \sigma^2$) ist definiert als der Mittelwert der quadrierten Abweichungen, $e_i^2$, (vom Mittelwert).$\square$
:::
...
Ein alternativer,
ähnlicher Kennwert zur mittlerer absolute Abweichung ist die SD.
...
p1 <- gghistogram(mariokart_no_extreme, x = "abw",
  ...
  labs(title = "Rohwerte") +
  annotate("label", x = 0, y = 20, label = "MW")
...
Der "gesunde Menschenverstand" würde spontan den mittleren Absolutabstand (MAA oder MAE) 
der Varianz (oder der Standardabweichung, SD) vorziehen. 
Das ist vernünftig, 
denn die MAA ist anschaulicher und damit nützlicher als die Varianz und die SD.
...
Gründe, warum Sie die Varianz (bzw. SD) kennen und nutzen sollten, sind:
...
- [mariokart-sd2](https://datenwerk.netlify.app/posts/mariokart-sd2/mariokart-sd2)
- [mariokart-sd3](https://datenwerk.netlify.app/posts/mariokart-sd3/mariokart-sd3)
- [Kennwert-robust](https://datenwerk.netlify.app/posts/kennwert-robust/kennwert-robust)
- [summarise04](https://datenwerk.netlify.app/posts/summarise04/summarise04)
- [summarise05](https://datenwerk.netlify.app/posts/summarise05/summarise05) 
- [vis-mariokart-variab](https://datenwerk.netlify.app/posts/vis-mariokart-variab/vis-mariokart-variab)
- [sd-vergleich](https://datenwerk.netlify.app/posts/sd-vergleich/sd-vergleich)
- [nasa01](https://datenwerk.netlify.app/posts/nasa01/nasa01)
- [Streuung-Histogramm](https://datenwerk.netlify.app/posts/streuung-histogramm/streuung-histogramm)
- [mariokart-sd1](https://datenwerk.netlify.app/posts/mariokart-sd1/mariokart-sd1)
- [summarise06](https://datenwerk.netlify.app/posts/summarise06/summarise06)
- [mariokart-desk01](https://datenwerk.netlify.app/posts/mariokart-desk01/mariokart-desk01)
...
:::: {layout="[ 80, 20 ]"}
::: {#first-column}
 Das ist die Forschungsfrage [dieser Umfrage](https://docs.google.com/forms/d/e/1FAIpQLSfM6oDLHX4_lqWq-bXw39drTkdGAvecE6ow2HIKoxdrVygp2A/viewform). Nehmen Sie ggf. an dieser Umfrage teil (sie ist anonym und dauert drei Minuten).
:::
...
Arbeiten Sie die die [Fallstudie "oecd-yacsda" im Datenwerk](https://datenwerk.netlify.app/posts/oecd-yacsda/) durch, 
