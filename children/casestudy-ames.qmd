...
@tbl-ames-lm1 zeigt die Parameter von `m1`.
...
Im Schnitt liegen wir `r rmse(m1)` Dollar daneben. 
...
Man kann mehrere UV (Prädiktorvariablen) in ein Regressionsmodell aufnehmen. Dazu trennt man sie mit einem Pluszeichen in `lm()`:
...
Im Schnitt liegen unsere Vorhersagen `r rmse(m2)` Dollar daneben. 
Ist das gut?
