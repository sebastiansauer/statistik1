...
Kennt man die beiden Parameter der Normalverteilung, Mittelwert und Streuung (SD, $\sigma$), 
einer Normalverteilung, 
so kann man einfach angeben, welcher Anteil der Fläche 
(unter der Kurve) der Normalverteilung sich in einem bestimmten Bereich befindet, 
s. @fig-norm-perc [@ainali_standard_2007].


Davon leitet sich die "68-95-99.7-Prozentregel" ab,
die angibt, in welchem Bereich sich welcher Anteil der Fläche (unter der Kurve einer Normalverteilung) befindet:
...
data <- read_csv("data/nonlinear_datasets.csv")
...
ggviolin(mariokart_no_extreme, 
         x = "cond", y = "total_pr", add = c("mean_sd", "jitter"), 
         add.params = list(shape = 1, alpha = .6)) 
```
