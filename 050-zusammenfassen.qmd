...
Welche Vorstellung haben Sie, wenn Sie hören, dass der "typische deutsche Mann" 1,80m groß ist [vgl. @owidhumanheight]? (Ihr Vorstellung updatet sich in @def-mw.)

a) Die Hälfte der Männer ist größer als 1,80&thinsp;m, die andere Hälfte kleiner.
b) Das arithmetische Mittel der Männer beträgt 1,80&thinsp;m.
c) Die meisten Männer sind 1,80&thinsp;m groß.
d) Etwas anderes.
...
Der Durchschnitt (das arithmetische Mittel, $\varnothing$, der Durchschnitt) beträgt: 2. $\square$
:::
...
sowie die Abweichungen der einzelnen Noten vom Mittelwert eingezeichnet.$\square$
:::
...
Haushalte in Deutschland in etwa ist (im Jahr 2021 auf Basis einer Umfrage)  [@deutsche_bundesbank_household_2023]?^[316500€] $\square$
...
sagen wir, es ist Kylian Mbappé^[Quelle: <https://www.transfermarkt.de/spieler-statistik/wertvollstespieler/marktwertetop>, Abruf 2023-03-19].
...
einmal mit (farbig markierten) Extremwerte (a) bzw. einmal ohne Extremwerte (b).
...
Der Befehl gibt als *Koeffizient* einen Wert zurück und zwar den Mittelwert von `einkommen`, vgl. auch @lst-einkommen.
...
#| fig-subcap: 
#|   - "Einkommen auf der Y-Achse"
#|   - "Einkommen auf der X-Achse"
#| echo: false
#| layout-ncol: 1
#| fig-asp: 0.5 
...
d %>% 
  ...
  annotate("label", x = 100, y = +1, label = "Dora") +
  annotate("label", x = 0, y = 1, label = "Anna\nBerta\nCarla", hjust = 0) +
  labs(caption = "MW: Mittelwert",
       y = "",
       x = "Einkommen")
```
...
:::{#exr-mw-wealthmd}
Was schätzen Sie, wie hoch das *mediane* Vermögen des Haushalte in Deutschland im Jahr 2021 in etwa war [@deutsche_bundesbank_household_2023]?^[ca. 83600€]

a) 50.000 Euro
b) 100.000 Euro
c) 150.000 Euro
d) 200.000 Euro
e) 300.00 Euro$\square$
:::
...
#| fig-cap: "Verschiedene Quantile visualisiert. In jedem Diagramm sind die Regionen gleich groß, beinhalten also (ungefähr) die gleiche Anzahl von Beobachtungen."
#| layout-ncol: 3
#| fig-subcap: 
#|  - "25%-Schritte: Quartile"
#|  - "10%-Schritte: Dezile"
