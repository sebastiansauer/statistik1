...
2. 📖 R und R-Befehle sind quelloffen, d.h. man kann sich die zugrundeliegenden Computerbefehle anschauen. Jede/r kann prüfen, ob R vernünftig arbeitet. Jede/r kann beitragen.
...
::: {.content-visible when-format="pdf"}
![So kann man R-Pakete installieren in RStudio](img/install-packages3.png){#fig-so-installieren}
...
RStudios GUI (Benutzeroberfläche) erlaubt es Ihnen auch, Daten per Klick, also ohne R-Befehle, zu importieren, s. @fig-daten-rstudio.



![Daten importieren per Klick](img/import-rstudio.png){#fig-daten-rstudio width=50%}

In der folgenden Maske können Sie unter `Browse` die zu importierende Datendatei auswählen. Mit Klick auf `Import` wird die Datei schließlich in R imporrtiert.

Sie können sowohl CSV-Dateien, Excel-Dateien oder Daten-Dateien aus anderen Statstik-Programmen (z.B. SPSS) importieren auf diese Weise.

Um eine CSV-Datei zu importieren, empfehle ich  die Option `From Text (readr)...`.
...
### Benötigte Daten dieses Kapitels
