...
```{mermaid}
%%| fig-cap: "Ihr Lernerfolg besteht aus drei Komponenten: Der Lehrkraft, Ihrer Mitarbeit im Unterricht und Ihrem Eigenstudium, d.h. Ihrer Vor- bzw. Nachbereitung zuhause."
...
Hm. Vielleicht fragen wir mal einen Chatbot wie ChatGPT, was es unter Statistik versteht:
...
:::{#def-statistik}
### Statistik
Statistik fasst Werte zusammen, quantifiziert deren Unterschiedlichkeit von und beschreibt die Ungewissheit unserer Schlüsse [@poldrack_statistical_2023; @kaplan_statistical_2009] . $\square$
:::
...
#| echo: false
#| layout-ncol: 2
#| fig-cap: "Die Ungewissheit, die wir Ereignissen zuschreiben, kann variieren"
...
:::{#exm-ungewiss2}
Sagen wir, Sie haben sich mit einem zwielichten Statistiker auf ein Glücksspiel eingelassen: Er wirft eine Münze 10 Mal; bei Kopf gewinnt er, bei Zahl sie. Nun hat der Staistiker von den 10 Würfen 8 Mal gewonnen. 
...
Die Arten von Zielen, die man mit einer statistichen Analyse verfolgen kann, lassen sich in drei Gruppen einteilen, s. @fig-ziele.
...
>   Um welchen Betrag erhöht sich der Umsatz (Y), wenn wir 1000€ mehr für Werbung ausgeben? (X)
...
aber implizit ist obige Hypothese herauszulesen (S. 142):
...
Beispiele für ID-Variablen sind z.B. Matrikelnummer, 
...
In @tbl-daten hat die Variable `name` drei Elemente mit den Werten Anna, Berta, Carla.
...
<!-- @fig-long-wide-anim zeigt einen Datensatz in der "tidy" Form, also tidy, und den gleichen Datensatz, umformatiert in der "breiten" Form, nicht-tidy. -->
...
```{r p-skalen-pdf}
#| echo: false
#| tbl-cap: "Erlaubte Rechenoperationen nach Skalenniveau (ja, erlaubt: ✓; nein, nicht erlaubt: ✕)"
#| label: tbl-skalenniveaus-pdf
skalenniveaus <- tibble::tribble(
       ~Skalenniveau, ~Quantitativ, ~`≠`, ~`$\\preceq$`, ~`+`, ~`×`,
...
Die Ordinalskale erlaubt also, Objekte zu ordnen (hinsichtlich eines Merkmals). Die Abstände zwischen den Objekten können dabei nicht quantifiziert werden. 
...
Stattdessen wurde mit  0° C eine Wärmenergiemenge gewählt, die für uns Menschen ganz praktisch, da augenfällig ist: der Gefrierpunkt von Wasser. 
...
ggplot(data_fig_intervall) +
  ...
  geom_label(aes(y = y, label = y), x = 0) + 
  theme_void() +
...
ist das das Bilden von Verhältnissen -- Multiplizieren (und damit auch Dividieren), s. @fig-verhaeltnis.
...
:::{#exr-skalenniveaus}
Überlegen Sie sich für einige Variablen die Skalenniveaus und befragen Sie dann interessierte Mitmenschen dazu. $\square$
:::
...
Puppen sind Modelle für Babies, Landkarten für Landstriche und das Atommodell von Nils Bohrist ein Modell für Atome. $\square$
:::
...
Die Aufgabe von Statistik ist es, durch Zusammenfassen von Daten Modelle zu bilden, 
die es uns einfacher machen, schwierige Sachverhalte zu verstehen. 
Zentral ist dabei die Analyse von Variabilität der Daten. 
Daten kommen in verschiedenen Varianten vor, 
typischerweise in Tabellenform, möglichst im Tidy-Format.
...
aber auch gewisse Nachteile und Schwäche;
...
Fassen Sie das *Wesentliche* (und nur das Wesentliche) zum Artikel an einem geeigneten Ort zusammen (z.B. auf einem Miro-Board).
$\square$
:::
::::
...
:::{#exr-xls-paper}
### Fassen Sie den Artikel von @broman_data_2018 zusammen
Fassen Sie das *Wesentliche* (und nur das Wesentliche) zum Artikel an einem geeigneten Ort zusammen .
$\square$
:::
::::
