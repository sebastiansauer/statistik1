...
benötigen Sie in diesem Kapitel noch `ggpubr` [@kassambara2023]
und optional `ggstatsplot` [@patil2021].
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/040-verbildlichen.html) (oder als [PDF](slides/040-verbildlichen.pdf)).
:::
...
![Der Zusammenhang von Entwicklungindex und Korruption [@wilke_wilkelabpracticalgg_2024]](img/develop-corrupt.png){#fig-develop-corrupt width=75%}
...
Eine einfache, komfortable ist die mit dem Paket `ggpubr`,
s. @fig-mario-n-plot-cond;
wir betrachten gleich die Syntax.
Zuerst importieren wir die Daten, s.  @lst-mariokart.
Außerdem nicht vergessen, das Paket `ggpubr` mit dem Befehl `library` zu starten.
...
`ggbarplot` erwartet -- anders als ein automatisches EDA-Tool -- fertige Balkenhöhen; daher zählen wir die Ausprägungen zuerst mit `count`.
...
Nimm den Datensatz `mariokart` *und dann*
  zähle die Ausprägungen der Spalte cond *und dann*
  zeichne ein Balkendiagramm daraus. Fertig!
```

:::{#exr-de1}
### Spalten wählen für das Balkendiagramm
Hätten wir eine andere Spalte gezählt und geplottet, so würde das Balkendiagramm die Verteilung jener Variablen zeigen.
...
mariokart |>
  count(stock_photo) |>
  ggbarplot(x = "stock_photo", y = "n")
```

Mit `ggbarplot` aus `ggpubr` kann man Balkendiagramme darstellen -- die Balkenhöhen (hier: Häufigkeiten je Ausprägung) müssen vorher mit `count` berechnet werden. $\square$
...
mariokart |>
  filter(total_pr < 100) |>  # ohne Extremwerte
  gghistogram(x = "total_pr", ggtheme = theme_minimal())

mariokart |>
  filter(total_pr < 100) |>  # ohne Extremwerte
  ggdensity(x = "total_pr", ggtheme = theme_minimal())
```
...
:::{#exr-plot-density}
Erstellen Sie das Diagramm @fig-de-hist-density, rechtes Teildiagramm!^[Grob gesagt: `mariokart |> ggdensity(x = "total_pr")`.]$\square$
:::
...
![Beispiele von Normalverteilungen mit verschiedenen Mittelwerten und Streuungen [@inductiveload2008]](img/normals.png){#fig-norms width=50% fig-align="center"}
...
:::: {layout="[ 80, 20 ]"}
::: {#first-column}
Die Entstehung einer Normalverteilung kann man gut anhand des [Galton-Bretts](https://www.youtube.com/watch?v=3m4bxse2JEQ) veranschaulichen.
:::
...
mariokart |> 
  ...
  geom_bar_pattern(
    ...
    pattern_fill = "grey30",
    pattern_color = "black") +
  scale_fill_manual(values = c(beta0col, beta1col)) +
...
z.$\,$B. mit `ggbarplot(x = "stock_photo", y = "n", fill = "cond", position = position_fill())` (Paket `ggpubr`), nachdem Sie die Häufigkeiten mit `count` berechnet haben.
...
mariokart |>
  ...
  count(wheels, cond) |>
  ggbarplot(x = "wheels", y = "n", fill = "cond",
...
@fig-streu1 links untersucht den Zusammenhang des Einstiegspreises (X-Achse) und Abschlusspreises (Y-Achse) von Geboten bei Versteigerungen des Computerspiels Mariokart.
...
Sie verbildlichen die Daten mit R, und zwar nutzen Sie das Paket `ggpubr`.
...
bauen Sie für jede davon einen eigenen Scatterplot (`ggscatter`) gegen den Abschlusspreis (`total_pr`),
da das die Variable ist, die Sie primär interessiert,
und fügen die einzelnen Diagramme mit `ggarrange` zu einem Panel zusammen.
...
c("n_bids", "start_pr") |>
  map(~ ggscatter(mariokart, x = .x, y = "total_pr")) |>
  ggarrange(plotlist = _, nrow = 1)
```
...
c("duration", "n_bids", "start_pr",
  "ship_pr", "seller_rate", "wheels") |>
  map(~ ggscatter(mariokart_no_extreme, x = .x, y = "total_pr")) |>
...
Wir bauen für jede X-Variable per `map` einen eigenen Scatterplot gegen `total_pr` und fügen alle mit `ggarrange` zu einem Panel zusammen. $\square$
...
#| eval: !expr knitr:::is_html_output()
mariokart_no_extreme |>
  ggscatter(x = "start_pr", y = "total_pr")
```

Man ruft `ggscatter` mit den gewünschten X- und Y-Variablen auf. $\square$
...
aber mit einer vereinfachten, gröberen Darstellungsform, 
...
Flink erstellen Sie dazu folgendes Diagramm, @fig-box-wheels1.
Es zeigt die Verteilung des Abschlusspreises, gruppiert nach Anzahl Lenkräder (`x = "wheels"`). $\square$
:::


`ggboxplot` gruppiert eine numerische X-Variable wie `wheels` automatisch nach jedem einzelnen vorkommenden Wert (`0`, `1`, `2`, `3`, `4`) -- ganz ohne dass wir sie extra in eine Faktorvariable umwandeln müssten.
Trotzdem ist es guter Stil, `wheels` explizit mit `factor(wheels)` (verpackt in `mutate`) als nominale Variable zu kennzeichnen:
Das dokumentiert Ihre Absicht und verhindert, dass spätere Analysen (z.$\,$B. Korrelationen oder Regressionsmodelle) `wheels` versehentlich als stetige Zahl behandeln.

```{r}
#| label: fig-box-wheels1
#| fig-cap: "Abschlusspreis nach Anzahl von beigelegten Lenkrädern"
mariokart_no_extreme |>
  mutate(wheels = factor(wheels)) |>  # wheels als nominale Variable kennzeichnen
  ggboxplot(x = "wheels", y = "total_pr")
```
...
#| out-width: 50%
mariokart_no_extreme |>
  mutate(ship_pr_grp = cut_interval(ship_pr, 5)) |>  # 5 Gruppen mit gleichen Ranges
  ggboxplot(x = "ship_pr_grp", y = "total_pr")
```

`ggboxplot` gruppiert -- anders als ein automatisches EDA-Tool -- keine metrische Variable von selbst.
Wir müssen `ship_pr` also explizit mit `cut_interval(ship_pr, 5)` in fünf Gruppen mit gleichen Ranges einteilen, bevor wir sie als Gruppierungsvariable nutzen können.
...
![Schokoladenkonsum und Nobelpreise [@messerli_chocolate_2012]](img/choc.jpeg){#fig-choc width="75%"}
...
dazu nutzt man den Parameter `palette = "black_first"`.
