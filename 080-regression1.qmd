...
Zuerst importieren Sie die Daten der letzten Klausur^[Diese Syntax wird bei Ihnen nur funktionieren, 
...
Je nach Datenlage können sich Regressionsgerade in Steigung oder Achsenabschnitt unterscheiden, 
s. @fig-regr-div.
...
auch den mittleren Quadratfehlerbalken (Mean Squared Error, MSE) berechnen^[Wer mag, 
...
Da es ein Anteilsmaß^[Prozentzahl] ist, 
...
daher ist dann auch $R^2 = 1$^[Bei Modellen mit einem Prädiktor; 
gibt es mehrere Prädiktoren gilt die Beziehung nur wenn die Prädiktoren alle paarweise unabhängig sind.], 
s. @fig-r2-extreme.
...
wie wenn man für jedes $y$ den Mittelwert, $\color{ycol}{\bar{y}}$, vorhersagen würde. 


:::{.callout-note}
Je größer R-Quadrat, desto besser erklärt das Modell die Daten 
...
um ca. 4 Euro.^[Die Spalte `95 CI` gibt einen Schätzbereich für den jeweiligen Modellkoeffizienten an,
...
Eine Aufgabe, die eine Einführung zum [Kaggle-Wettbewerb Ames House Prices](https://www.kaggle.com/competitions/house-prices-advanced-regression-techniques/overview) bietet, 
finden Sie [hier im Datenwerk](https://datenwerk.netlify.app/posts/ames-kaggle1/ames-kaggle1.html).
