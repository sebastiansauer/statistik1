...
```{r lst-summarise, lst-cap: "Die R-Funktion summarise fasst einen Vektor z u einer Zahl zusammen"}
...
>   🧑‍🎓  Hey R, fasse die Zeilen von `total_pr` aus `mariokart` zu einer Zahl zusammen, und zwar mit Hilfe des Mittelwerts. Die resultierende Tabelle nennen wir `mariokart_neu, sehr kreativ. Und die resultierende Spalte, die einzige in `mariokart_neu`, nennen wir `preis_mw`.
...
[Hier](https://www.statology.org/dplyr-case_when/) - und an vielen weiteren Stellen im Internet - finden Sie ein Tutorium.
