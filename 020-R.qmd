...
knitr::opts_chunk$set(tidy = TRUE, width = 60, max.print = 100)
```
...
>   [🧑‍🎓]{.content-visible when-format="html"} Wie ist es um unsere Wissenschaft, Psychologie, bestellt? Haben die Befunde Hand und Fuß?
...
>   [🧑‍🎓]{.content-visible when-format="html"} Welche R-Pakete sind denn schon installiert?
...
>   [🧑‍🎓]{.content-visible when-format="html"} Ja, aber welche R-Pakete "soll" ich denn installieren, welche brauch ich denn?
...
R meint es gut mit Ihnen^[>   [🤖]{.content-visible when-format="html"} Naja, manchmal.]. Stellen Sie sich vor, 
dass R Sie auf dieses Problem aufmerksam machen möchte: 

>    [🤖]{.content-visible when-format="html"} Achtung, lieber Herr und Gebieter, du hast nicht mehr alle Latten am Zaun, will sagen, alle Daten im Vektor!
...
:::{#exr-na}
### Geben Sie neue Bedeutungen an, was "NA" noch bedeuten könnte!

>    [🤖]{.content-visible when-format="html"} Wie wäre es mit "nebulöse Anomalie" oder "nix-checkender Angeber" oder "nölender Automat".

>    [👩‍🎓]{.content-visible when-format="html"} Hm...
...
:::{#exr-rquiz}
Ihre R-Muskeln sind gestählt? [💪]{.content-visible when-format="html"} Oder doch noch nicht so ganz ausdefiniert? [😤]{{.content-visible when-format="html"}} Macht nichts! Trainieren Sie sich mit dem R-Quiz auf der Datenwerk-Webseite^[<https://datenwerk.netlify.app/posts/r-quiz/r-quiz>]! $\square$
:::
...
In beiden Fällen wird der "Aufenthaltsort" der Datei durch den *Pfad*^[Der Pfad einer Datei sagt, in welchem Ordner und Unterorder und Unter-Unterordner die gesuchte Datei liegt. Ein Pfad könnte z.B. so aussehen: "/Users/sebastiansaueruser/github-repos/statistik1/".] und den Namen der Datei definiert.
...
Auf der anderen Seite ist `d` nicht gerade präzise und vielsagend.
...
>   [🧑‍🎓]{.content-visible when-format="html"} Wer will schon wieder wen prüfen?!
...
>   [🤖]{.content-visible when-format="html"} Hallo, Mensch. Ja, diese Variable hat den Wert 42.
...
>   [🧑‍🎓]{.content-visible when-format="html"} Aber ist R wirklich ein Werkzeug, das mir im Job hilft? 
...
>    [👩]{.content-visible when-format="html"}  R ist *der* Place-to-be für die Datenanalyse.


>    [🧑‍🎓]{.content-visible when-format="html"}  Aber ist Datenanalyse wirklich etwas, womit ich in Zukunft einen guten Job bekomme?
...
Noch nicht genug? Checken Sie alle Aufgaben mit dem Tag [R](https://datenwerk.netlify.app/#category=R) auf dem Datenwerk aus.^[<https://datenwerk.netlify.app/#category=R>] 
...
```{r}
#| eval: false
#|  # Das Paket muss installiert sein:
data(mariokart, package = "openintro") 
```
