...
## Forschungsbezug: Wie man Ihre Persönlichkeit durch Ihr Facebook-Profil vorhersagen kann
...
:::{#fig-wetter-anim layout-ncols=2}
![Temperatur (Grad Celcius) im Verlauf der Jahre](img/fig-wetter-anim-1.gif)

![Niederschlage (mm) im Verlauf der Jahre](img/fig-wetter-anim-2.gif)

Veränderung der Temperatur und Niederschlag (10-Jahres-Mittel) in Deutschland
:::
...
wetter %>% 
  ...
  geom_hline(yintercept = coef(lm_wetter1)[1], linetype = "dashed", color =okabeito_colors()[2]) +
  coord_cartesian(ylim = c(7, 9))  +
  annotate("label", x = Inf, y = coef(lm_wetter1)[1], 
           hjust = "right", label = paste0("b0"), color =okabeito_colors()[2]) +
  geom_segment(x = 1, y = coef(lm_wetter1)[1], xend = 1, 
               yend = coef(lm_wetter1)[1] + coef(lm_wetter1)[2], 
               color = okabeito_colors()[1], arrow = arrow()) +
  annotate("label", x = 1 , y = coef(lm_wetter1)[1] + (coef(lm_wetter1)[2] / 2), 
           hjust = "right", label = paste0("b1"), color = okabeito_colors()[1])  +
...
So sieht dann die geänderte Reihenfolge aus:^[Zum Dollar-Operator s. @dollar-op]:
...
![Veränderung der Temperatur pro Monat in Deutschland im Zeitverlauf (10-Jahres-Mittelwerte)](img/fig-temp-monat-1.gif){#fig-temp-monat}
```{r}
#| echo: false
#| eval: false
...
Mit `scale_color_okabeito` haben wir die Standard-Farbpalette durch die von  [@okabeito] ersetzt (s. [Hinweise hier](https://malcolmbarrett.github.io/ggokabeito/reference/palette_okabe_ito.html)).
...
<!-- ```{mermaid} -->
<!-- %%| fig-cap: Unterteilung dieser Stichproben-Arten -->
<!-- %%| label: fig-samples -->
<!-- graph TD -->
<!--   D[Daten] --> Train[Train-Sample] -->
<!--   D[Daten] --> Test[Test-Sample] -->
<!--   Train --> V[Validation-Sample] -->
<!--   Train --> A[Assessment-Sample] -->
<!-- ``` -->
...
:::{#def-trainsample}
### Train-Sample
Den Datensatz, für die Sie sowohl UV *als auch AV* vorliegen haben, nennt man Train-Sample.
:::
...
:::{#def-testsample}
### Test-Sample
Den Datensatz, für den Sie *nur* Daten der UV, aber nicht zu der AV vorliegen haben, nennt man *Test-Sample*.
:::
