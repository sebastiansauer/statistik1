...
Ein bisschen abstrakter gesprochen, fasst `summarise` also eine *Spalte* zu einer (einzelnen) *Zahl* zusammen, s. @eq-desk-summ.^[Eine Alternative, um eine Spalte zu einer Zahl zusammenzufassen, bietet der "Dollar-Operator" (`$`: 
...
So sieht dann Ihre Befehlssequenz mit der Pfeife ("%>%") aus:

```{r}
# Daten einlesen:
mariokart <- read.csv("https://vincentarelbundock.github.io/Rdatasets/csv/openintro/mariokart.csv")

# R-Paket tidyverse, darin wohnen die Pfeife und die Tidyverse-Verben:
library(tidyverse)

mariokart_pfeifentest <- 
mariokart %>%  # Nimm die Tabelle mariokart und dann ... 
  filter(total_pr < 100)  %>%  # filtere die günstigen Spiele und dann ...
  select(total_pr, cond)  %>%   # wähle die benötigen Spalten und dann ...
  group_by(cond) %>%  # gruppiere nach Zustand (neu/alt) und dann ...
  summarise(pr_mean = mean(total_pr))  # fasse zusammen zum mittleren Verkaufspreis
