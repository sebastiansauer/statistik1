...
Vielleicht fordert Sie die Lehrkraft zu einem Einstiegsquiz auf, etwas mittels der Plattform [antworte.jetzt](https://antworte.jetzt/).^[<https://antworte.jetzt/>] 
...
Laut dem *Harvard Business Review* allerdings,
verbringen Data Scientist "80%" ihrer Zeit mit dem *Aufbereiten* von Daten [@bowne-anderson_what_2018].
...
![Mach's einfach [@imgflip_imageflip_2024] ](img/thatwouldbegreat.jpg){#fig-that-would-be-great width="50%"}

Der Trick besteht darin, komplexe Operationen in mehrere einfache Teilschritte zu zergliedern. (In gewisser Weise besteht das Wesen einer Analyse eben darin: die Zerlegung eines Objekts in seine Bestandteile.)
...
:::{.callout-important}
Die Verben des Datenjudos wohnen im Paket `dpylr`,
...
```{r}
#| tbl-cap: Die Datentabelle, soritert nach `total_pr`
#| label: tbl-arrange
...
#| label: fig-summarise
#| fig-asp: 0.5
#| fig-cap: "Spalten zu einer einzelnen Zahl zusammenfassen mit `summarise()`"
d_summ <-
  d %>% 
...
Ein bisschen abstrakter gesprochen, fasst `summarise` also eine *Spalte* zu einer (einzelnen) *Zahl* zusammen, s. @eq-desk-summ.
...
liefert der Dollar-Operator einen Vektor (Spalte) zurück. (Diese wird von `mean` dann zu einer einzelnen Zahl zusammengefasst.) $\square$
:::
...
Langsam fühlen Sie sich als Datenchecker ... [🥷 🦹‍♀]{.content-visible when-format="html"}] $\square$
:::
...
[🚬 👈 ]{.content-visible when-format="html"}Das ist keine Pfeife, 
wie René Magritte 1929 in seinem [berühmten Bild](https://en.wikipedia.org/wiki/File:MagrittePipe.jpg) schrieb, 
s. @fig-pfeifen [@m7_savinellis_2004-1].^[Vgl. <https://en.wikipedia.org/wiki/The_Treachery_of_Images>]
...
:::{.callout-caution}
Ein beliebter Fehler ist es übrigens, nicht die richtige Zahl an schließenden Klammern hinzuschreiben, z.B. `d(c(b(a(meine_daten))` FALSCHE ZAHL AN KLAMMERN. $\square$
:::
...
dass der Shortcut Strg-Shift-M die eingebaute Pfeife verwendet.
...
Aus dieser Tabellet sehen wir, dass 3 oder 4 Lenkräder nur selten (2 bzw. 1 Mal) 
beigelegt wurden und wir solche Spiele herausfiltern sollten, 
bevor wir den Mittelwert der Versankosten ausrechnen:
...
Beispielhaft ist unter <https://osf.io/4b9p2> die R-Syntax zu sehen, die die Autoren zur Datenaufbereitung verwendet haben.
...
![Studie COVIDiSTTRESS [@lieberoth_covidistress_2020]](img/Covidistress1.jpg){width=30%}
<https://osf.io/z39us/>
...
[Hier](https://www.statology.org/dplyr-case_when/) -- und an vielen weiteren Stellen im Internet -- finden Sie ein Tutorium.^[<https://www.statology.org/dplyr-case_when/>]
...
<center>
![Help me help you [@imgflip_imageflip_2024]](img/help-me-help-you.gif){width=50%}
</center>
:::
...
Er ist einer der Hauptautoren von  beliebten R-Paketen wie `dplyr` und `ggplot2`.
