...
In die gleiche Bresche schlägt "Anscombes Quartett", s. @fig-dino2:
Es zeigt 4 Datensätze, in denen die zentralen Statistiken fast identisch sind,
...
[Quelle: Plotly](https://plotly.com/r/trisurf/)
...
Außerdem nicht vergessen, das Paket `DataExplorer` zu starten, 
s. @lst-dataexplorer.^[Natürlich müssen Sie das Paket einmalig installiert haben, bevor Sie es starten können.]
...
@fig-plot-distribs zeigt verschiedene Formen von Verteilungen. "Bimodal" meint "zweigipflig" und "multimodal" entsprechend "mehrgipflig".
...
Kennt man diese beiden Parameter, so kann man einfach angeben, welcher Anteil der Fläche sich in einem bestimmten Bereich befindet, s. @fig-norm-perc.
...
p1 <- ggplot(data, aes(x=x1, y=y1)) + geom_point() + ggtitle("Quadratischer Zusammenhang") + theme_minimal()
p2 <- ggplot(data, aes(x=x2, y=y2)) + geom_point() + ggtitle("Exponenzieller Zusammenhang") + theme_minimal()
p3 <- ggplot(data, aes(x=x3, y=y3)) + geom_point() + ggtitle("Sinus-Zusammenhang") + theme_minimal()
p4 <- ggplot(data, aes(x=x4, y=y4)) + geom_point() + ggtitle("Logarithmischer Zusammenhang") + theme_minimal()
...
Zwar sind sie beliebt, doch ist [von ihrer Verwendung zumeist abzuraten](https://www.data-to-viz.com/caveat/pie.html); vgl. auch [hier](https://github.com/cxli233/FriendsDontLetFriends#10-friends-dont-let-friends-make-pie-chart).
...
Der Quellcode der Animation ist [hier](https://gist.github.com/rafapereirabr/0d68f7ccfc3af1680c4c8353cf9ab345) zu finden.
...
Ein weiteres R-Paket zur Erstellung ansprechender Datenvisualisierung heißt [`ggstatsplot`](https://github.com/IndrajeetPatil/ggstatsplot/blob/main/README.md).

@fig-ggstatsplot zeigt ein [Histogramm](https://github.com/IndrajeetPatil/ggstatsplot/blob/main/README.md#gghistostats), das mit `ggstatsplot` erstellt wurde.
...
Einige Überlegungen zur Farbwahl findet sich [in diesem Post](https://data-se.netlify.app/2023/06/30/farbpaletten/); ausführlichere Erläuterung bietet @wilke_fundamentals_2019, s. [Kap. 4](https://clauswilke.com/dataviz/color-basics.html).

So ist die Farbpalette von Okabe und Ito [vgl. @ichihara_color_2008] empfehlenswert, da sie auch bei Schwarz-Weiß-Druck und bei Sehschwächen die Farben noch recht gut unterscheiden lässt, s. @fig-okabe
