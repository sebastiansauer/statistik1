...
wetter_path <- paste0(
  "https://raw.githubusercontent.com/sebastiansauer/data_se/refs/heads/master/content/post/",
  "2026-08-14-wetterdaten-des-dwd-aufbereiten/wetterdaten_monat.csv")
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/090-regression2.html) (oder als [PDF](slides/090-regression2.pdf)).
:::
...
#| echo: false
#| warning: false
#| fig-cap: "Temperatur (Grad Celsius) im Verlauf der Jahre"

wetter_temp_jahr <- wetter |>
  filter(region == "Deutschland") |>
  summarise(temp = mean(temp), .by = year)

ggplot(wetter_temp_jahr, aes(x = year, y = temp)) +
  geom_line(color = beta1col) +
  geom_smooth(color = errorcol, se = FALSE) +
  labs(x = "Jahr", y = "Temperatur (°C)")
```
...
#| echo: false
#| warning: false
#| fig-cap: "Niederschlag (mm) im Verlauf der Jahre"

wetter_precip_jahr <- wetter |>
  filter(region == "Deutschland") |>
  summarise(precip = mean(precip), .by = year)

ggplot(wetter_precip_jahr, aes(x = year, y = precip)) +
  geom_line(color = beta1col) +
  geom_smooth(color = errorcol, se = FALSE) +
  labs(x = "Jahr", y = "Niederschlag (mm)")
```

### Monatstemperaturverlauf

```{r fig-wetter-temp-monat}
#| echo: false
#| warning: false
#| fig-cap: "Veränderung der Temperatur pro Monat in Deutschland im Zeitverlauf"

wetter_temp_monat <- wetter |>
  filter(region == "Deutschland") |>
  summarise(temp = mean(temp), .by = c(year, month))

ggplot(wetter_temp_monat, aes(x = year, y = temp, color = factor(month))) +
  geom_line() +
  geom_smooth(aes(group = 1), color = "black", se = FALSE) +
  labs(x = "Jahr", y = "Temperatur (°C)", color = "Monat")
```

### Monatsniederschlagsverlauf

```{r fig-wetter-precip-monat}
#| echo: false
#| warning: false
#| fig-cap: "Veränderung des Niederschlags pro Monat in Deutschland im Zeitverlauf"

wetter_precip_monat <- wetter |>
  filter(region == "Deutschland") |>
  summarise(precip = mean(precip), .by = c(year, month))

ggplot(wetter_precip_monat, aes(x = year, y = precip, color = factor(month))) +
  geom_line() +
  geom_smooth(aes(group = 1), color = "black", se = FALSE) +
  labs(x = "Jahr", y = "Niederschlag (mm)", color = "Monat")
```
...
Weitere Auswertungen zum Klimaverlauf bzw. Klimawandel finden Sie z.B. [hier](https://sebastiansauer.github.io/data_se/2026/08/14/wetterdaten-des-dwd-analysieren-2026/).
...
>    [🧑‍🏫]{.content-visible when-format="html"}[\emoji{teacher}]{.content-visible when-format="pdf"} Um wie viel ist die Temperatur in Deutschland pro Jahr gestiegen, wenn man die letzten ca. 100 Jahre betrachtet?
...
Laut dem Modell wurde es pro Jahr im Schnitt um `r round(coef(lm_wetter1)["year"], 2)`$\,$°C wärmer, pro Jahrzehnt also `r round(coef(lm_wetter1)["year"] * 10, 2)`$\,$°C und pro Jahrhundert `r round(coef(lm_wetter1)["year"] * 100, 2)`$\,$°C.
...
2. *Bereichsschätzungen* Cleverer als Punktschätzungen sind Bereichsschätzungen (Intervallschätzungen): Hier wird ein Bereich plausibler Werte für den entsprechenden Koeffizienten angegeben. In der Spalte `CI` sehen Sie die untere bzw. die obere Grenze eines "Bereichs plausibler Werte". Dieser Schätzbereich wird auch als *Konfidenzintervall* (engl. confidence interval, CI) bezeichnet.
Ein Konfidenzintervall ist mit einer Sicherheit zwischen 0 und 1 angegeben, z.B. 95$\,$% (0.95).
Grob gesagt bedeutet ein 95$\,$%-Konfidenzintervall, dass wir uns (laut Modell) zu 95$\,$% sicher sein können, dass der wahre Wert sich in diesem Bereich befindet.
In @tbl-lm-wetter1 können wir ablesen, dass das Regressionsgewicht von `year` irgendwo zwischen praktisch Null (`r round(confint(lm_wetter1)["year", 1], 3)`)
und ca. `r round(confint(lm_wetter1)["year", 2], 3)` Grad geschätzt wird.
...
Im Jahr 1951 (x=0) lag die mittlere Temperatur in Deutschland (laut DWD) bei ca. `r round(coef(lm_wetter1_zentriert)["(Intercept)"], 1)`$\,$°C.
Die Regressionsgleichung lautet: *temp_pred = `r round(coef(lm_wetter1_zentriert)["(Intercept)"], 2)` + `r round(coef(lm_wetter1_zentriert)["year_c"], 2)`\*year_c*.
In Worten: Wir sagen eine Temperatur vorher, die sich als Summe von `r round(coef(lm_wetter1_zentriert)["(Intercept)"], 2)`$\,$°C plus `r round(coef(lm_wetter1_zentriert)["year_c"], 2)` mal das Jahr (in zentrierter Form) berechnet.
...
und zwar offenbar um `r round(coef(lm_wetter_bin_uv)["after_1950TRUE"], 2)` Grad, s. @fig-wetter2.
...
#| label: fig-wetter2
#| layout: [[45,-10, 45], [100]]
#| fig-cap: !expr 'sprintf("Modell: `temp ~ after_1950`, (a) Der Schätzbereich für den Parameter reicht von ca. %.2f bis %.2f Grad Unterschied. (b) Der Unterschied sieht in dieser Darstellung nicht groß aus.", confint(lm_wetter_bin_uv)["after_1950TRUE", 1], confint(lm_wetter_bin_uv)["after_1950TRUE", 2])'
#| fig-subcap: 
...
Umwandlung in eine oder mehrere *binäre* Variablen, s. @def-binvar.
...
- Temperatur laut Modell bis 1950: $\color{modelcol}{\hat{y}} = \color{beta0col}{\beta_0} = `r round(coef(lm4)[1], 1)`$

- Temperatur laut Modell nach 1950: $\color{modelcol}{\hat{y}} = \color{beta0col}{\beta_0} +  \color{beta1col}{\beta_1}= \color{beta0col}{`r round(coef(lm4)[1], 1)`} + \color{beta1col}{`r round(coef(lm4)[2], 1)`} = `r round(coef(lm4)[1] + coef(lm4)[2], 1)`$
:::
...
- Temperatur laut Modell bis 1950: ${\hat{y}} = {\beta_0} = `r round(coef(lm4)[1], 1)`$

- Temperatur laut Modell nach 1950: ${\hat{y}} = {\beta_0} + {\beta_1}= {`r round(coef(lm4)[1], 1)`} + {`r round(coef(lm4)[2], 1)`} = `r round(coef(lm4)[1] + coef(lm4)[2], 1)`$
::::
...
*zwei* Faktorstufen besitzt (`"f"` und `"m"`).
...
1. Achsenabschnitt ($\beta_0$, `Intercept`): Im Referenzjahr (1951) im *Referenzmonat Januar* lag die Niederschlagsmenge bei `r round(coef(lm_year_month)["(Intercept)"], 0)`$\,$mm pro Quadratmeter.
2. Regressionskoeffizient für Jahr ($\beta_1$, `year_c`): Pro Jahr ist die Niederschlagsmenge im Schnitt um `r round(coef(lm_year_month)["year_c"], 2)`$\,$mm an (im Referenzmonat).
3. Regressionskoeffizient für Monat ($\beta_2$, `month_factor7`) Im Monat `7` (Juli) lag die mittlere Niederschlagsmenge (im Referenzjahr) knapp `r round(coef(lm_year_month)["month_factor7"], 0)`$\,$mm über dem mittleren Wert des Referenzmonats (Januar).


Die Regressionsgleichung von `lm_year_month` lautet:
`r sprintf("precip_pred = %.2f + %.2f*year_c + %.2f*month_factor_7", coef(lm_year_month)["(Intercept)"], coef(lm_year_month)["year_c"], coef(lm_year_month)["month_factor7"])`.
Im Monat Juli ist `month_factor_7 = 1`,
ansonsten (Januar) ist `month_factor = 0`.
Demnach erwarten wir laut Modell `lm_year_month` im Juli des Referenzjahres `r round(coef(lm_year_month)["(Intercept)"] + coef(lm_year_month)["month_factor7"], 2)`$\,$mm Niederschlag.
...
In diesem Fall ist der Interaktionseffekt ungleich Null. $\square$
...
Pro Jahr ist die Zunahme an Niederschlag um `r round(abs(coef(lm_year_month_interaktion)["year_c:month_factor7"]), 2)`$\,$mm geringer
als im Referenzmonat (Januar).
Damit resultiert für Juli insgesamt ein `r ifelse(coef(lm_year_month_interaktion)["year_c"] + coef(lm_year_month_interaktion)["year_c:month_factor7"] < 0, "negativer", "positiver")` Effekt:
`r sprintf("%.2f + (%.2f) = %.2f", coef(lm_year_month_interaktion)["year_c"], coef(lm_year_month_interaktion)["year_c:month_factor7"], coef(lm_year_month_interaktion)["year_c"] + coef(lm_year_month_interaktion)["year_c:month_factor7"])`.
Insgesamt lautet die Regressionsgleichung:
`r sprintf("precip_pred = %.2f + %.2f * year_c + %.2f * month_factor_7 + (%.2f) * year_c:month_factor_7", coef(lm_year_month_interaktion)["(Intercept)"], coef(lm_year_month_interaktion)["year_c"], coef(lm_year_month_interaktion)["month_factor7"], coef(lm_year_month_interaktion)["year_c:month_factor7"])`.
...
![Animation eines Regressionsmodells mit zwei metrischen UV, x1 und x2; y wird gut von den beiden UV erklärt](img/regression_plane_rotation.gif){#fig-regression-plane-rotation}
...
Den ersten Teil kannst du nutzen, um Modelle zu berechnen ("trainieren") und ihre Güte zu prüfen. Den Teil nenne ich "Train-Sample", hört sich cool an, oder? 
...
`rsq` kann die Modellgüte für beliebige Vorhersagewerte berechnen,
...
Es gibt einen *positiven* Zusammenhang von Schnabellänge und Schnabeltiefe, s. @fig-penguins-groups.

```{r}
#| fig-cap: "Der Zusammenhang von Schnabellänge und Schnabeltiefe pro Gruppe von Pinguinen: Die Regressionsgerade pro Gruppe steigt. Hingegen sinkt die Regressionsgerade ohne Beachtung der Gruppen (schwarze gestrichelte Linie)"
...
Nicht-experimentelle Studien können zu grundverschiedenen Ergebnissen führen,
...
beispielhaft zwei ausführlichere Entwicklungen eines Prognosemodells.
