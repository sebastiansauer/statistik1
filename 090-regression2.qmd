...
@fig-wetter-anim zeigen die Wetterdaten - Niederschlag und Temperatur in Deutschland.
...
#| echo: false
#| warning: false
#| fig-cap: "Niederschlag pro Monat (mm) im Verlauf der Jahre"
...
p_wetter_temp_monat <- ggplot(wetter_temp_monat, aes(x = year, y = temp, color = factor(month))) +
  geom_line() +
...
ggplotly(p_wetter_temp_monat)
```

```{r fig-wetter-temp-monat-pdf}
#| echo: false
#| warning: false
...
ggplot(wetter_month_1_7) +
  aes(x = year_c, 
      ...
      group = month_factor) +
  geom_point2(alpha = .2, aes(shape = month_factor)) + 
  geom_smooth(method = "lm", size = 2, fullrange = TRUE) +
  scale_color_manual(values = c("1" = beta0col, "7" = beta1col)) +
...
plot(lm3d_expect, show_data = TRUE)
```
