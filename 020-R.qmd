...
>    [🧑‍🎓]{.content-visible when-format="html"}[\emoji{student}]{.content-visible when-format="pdf"} Wie ist es um meine Wissenschaft, Psychologie, bestellt? Haben die Befunde Hand und Fuß?
...
Anstelle von *RStudio Desktop* (auf Ihrem Computer/Desktop) können Sie auch die *RStudio Cloud* (die Online-Version) starten. $\square$
:::
...
der bestimmte Werte enthält, z.B. den Wert "9° Celcius".
...
bevor man es nutzen kann. $\square$
:::
...
Der Befehl `read.csv` bietet eine Möglichkeit, Daten (in Form einer Tabelle) von einer Webseite (URL) in R zu importieren:
...
Eine weitere Möglichkeit, um Daten von einem Ordner (egal ob dieser sich im Internet oder auf Ihrem Computer befinde) einzulesen, stellt die Funktion `data_read` bereit:
