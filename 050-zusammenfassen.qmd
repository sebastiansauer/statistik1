...
:::{#exr-mw-md-mod}
Welche Vorstellung haben Sie, wenn Sie hören, dass der "typische deutsche Mann" 1,80m groß ist [@owidhumanheight]?
...
Mit $\hat{y}$ sind die vorhergesagten bzw. die zu erklärende Variable^[AV, Output-Variable, Zielvariable] gemeint.
...
Vier Studentis Anna, Berta, Carl, Dani haben ihre Statistik-Klausur zurückbekommen.
...
In R kann man das mittlere Einkommen (präziser: das arithmetische Mittel des Einkommenss) wie folgt berechnen.^[Die Details der Syntax, z.B. der Befehl `rep()` sind von geringer Bedeutung.]
...
:::{#def-lm}
### Lineares Modell
Ein lineares Modell verwendet eine Gerade als Modell der Daten.$\square$
:::
...
Der Befehl gibt als *Koeffizient* einen Wert zurück und zwar den Mittelwert von `einkommen`, s. oben.
Dieser Wert wird als Achsenabschnitt (engl. *intercept*) bezeichnet,
das wird verständlich, wenn man z.B. in @fig-mw2 sieht, dass die Gerade genau an diesem Punkt die Y-Achse schneidet.
...
Ein Quantil ist also ein Oberbegriff für Quantile, Dezile, etc.
...
@fig-mw2 zeigt ein Beispiel für ungruppierte (links) bzw. gruppierte (rechts) Mittelwerte.
...
3. Mit KommilitonInnen austauschen oder in höheren Semestern nach Tipps fragen.$\square$
:::
