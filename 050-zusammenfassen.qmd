...
library(ggplot2)
theme_update(axis.title = element_text(size = 66))
```
...
Laut [dieser Quelle](https://en.wikipedia.org/wiki/Average_human_height_by_country) beträgt der Wert der mittleren Größe deutscher Frauen etwa 1,66m, also 14&thinsp;cm weniger als bei Männern.^[<https://en.wikipedia.org/wiki/Average_human_height_by_country>] $\square$
:::
...
:::{#def-mw}
### Mittelwert
Der Mittelwert  der Variablen $X$ (präziser: das arithmetische Mittel von $X$) ist definiert als die Summe der Elemente von $X$ geteilt durch deren Anzahl, $n$. Den Mittelwert von $X$ bezeichnet man mit $\bar {x}$. $\square$
:::
...
Bezeichnen wir die Abweichung - auch als "Fehler", "Rest" oder "Residuum" bezeichnet - der $i$-ten Person mit $\color{errorcol}{\text{e}_i}$  (*e* wie engl. *error*, "Fehler) und die $i$-te Note mit $\color{ycol}{y_i}$, so können wir mit @eq-modell1 festhalten:
...
Der tatsächliche, beobachtete Wert $\color{ycol}{\text{y}}$ setzt sich zusammen aus dem Modellwert $\color{modelcol}{\text{m}}$ plus einem Fehler $\color{errorcol}{\text{e}}$, s. @eq-modell3.

$$\color{ycol}{\text{y}} \color{black}{ = } \color{modelcol}{\text{m}} + \color{errorcol}{\text{e}}$${#eq-modell3}
...
so würden wir schreiben (s. @eq-modell5):

$$\color{modelcol}{\bar{y}} \color{black}{ \sim } \color{xcol}{\text{x}} $$
\color{ycol}{\bar{y}}{#eq-modell4}
Da wir im Moment aber keine andere Variablen bemühen, um $\color{ycol}{\text{y}}$ zu erklären, schreibt man mit @eq-modell5 auch:
...
:::{#exr-mw-wealth1}
Was schätzen Sie, wie hoch das "mittlere" Vermögen des Haushalte in Deutschland in etwa ist?^[Quelle:  SI, <https://www.wsi.de/en/how-is-wealth-distributed-in-germany-14401.html>, Abruf 2023-04-19]. $\square$
...
Kommt der wertvollste Fußballspieler der Welt in Ihren Hörsaal, sagen wir, es ist Kylian Mbappé^[Quelle: <https://www.transfermarkt.de/spieler-statistik/wertvollstespieler/marktwertetop>, Abruf 2023-03-19].
Sein Jahreseinkommen (2023) liegt bei ca. 120 Millionen Euro^[Quelle: <https://www.einkommenmagazin.de/kylian-mbappe-einkommen/>, Abruf 2023-03-19].
...
In R kann man das mittlere Einkommen (präziser: das arithmetische Mittel des Einkommens) wie folgt berechnen, s. @lst-einkommen.^[Die Details der Syntax, z.B. der Befehl `rep()`, sind von geringer Bedeutung.]
...
<!-- Alterantiv zu @lst-mario-lage ließe sich auch der Befehl `describe_distribution()` aus `{easystats}` verwenden:  -->
...
:::{.callout-tip}
Schauen Sie sich auch mal auf [datenwerk.netlify.app](https://datenwerk.netlify.app) die Aufgaben zu z.B. dem Tag [EDA](https://datenwerk.netlify.app/#category=eda) an. $\square$
:::


:::{#exr-datensaetze}
Mittlerweile verfügen Sie die wesentlichen Werkzeuge des Datenjudo. [Hier](https://data-se.netlify.app/2022/02/23/data-sets-for-for-teaching/) finden Sie einen Überblick an Datensätze, die Sie nach Herzenslust analysieren können.^[<https://data-se.netlify.app/2022/02/23/data-sets-for-for-teaching/>] $\square$
:::
