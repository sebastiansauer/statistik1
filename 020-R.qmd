...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/020-R.html) (oder als [PDF](slides/020-R.pdf)).
:::
...
In diesem Buch stellen wir Ihnen zwei gleichwertige Optionen vor: 
*RStudio* und *Positron*, beide von der Firma Posit entwickelt.
...
Sie können R-Pakete installieren mit dem Befehl `install.packags()`; das geht in jeder IDE.
Außerdem kann man Pakete per Klick in der IDE installieren.
...
![So kann man R-Pakete installieren in RStudio](img/install-pckgs.gif){#fig-so-installieren-gif width="75%"}

### Positron

![So kann man R-Pakete installieren in Positron](img/install-pckgs-positron.png){#fig-so-installieren-positron width="75%"}
...
auf Ihrem Computer schon installiert sind; in Positron klicken Sie links im blauen Balken auf das "Paketsymbol".
...
:::
![Positron: Ordner öffnen](img/positron-open-folder.png)
:::
...
Nutzen Sie `File > New File > R Script`, oder das Tastaturkürzel `Strg+N`. 
Der Menüpfad ist zu RStudio analog, das Icon-Set sieht in Positron aber anders aus.

:::
![Positron: Neue (Skript-)Datei erstellen](img/neue-skriptdatei-positron.png)
:::
...
Quarto ist eine komfortable und leistungsfähige Methode,
um Dokumente mit R-Syntax anzureichern.
...
:::{.callout-note}
Seit Version 2026.09 bietet auch Positron einen Klick-Import: Öffnen Sie die zu importierende Datei (CSV, TSV, Excel oder Parquet), z. B. über den Data Explorer, das Datei-Menü, das Variables-Pane oder per Rechtsklick im Datei-Explorer, und klicken Sie dort auf `Import Data`. Es öffnet sich ein Dialog, der den passenden R-Code zum Einlesen anzeigt (`readr` für CSV/TSV, `readxl` für Excel, `nanoparquet` für Parquet); ein Klick auf `Import` führt diesen Code in der Konsole aus, s. @fig-daten-positron. Anders als bei RStudio zeigt Positron dabei aber keine Vorschau der Tabelle mit Optionen zu Trennzeichen, Kopfzeile oder Spaltentypen -- dafür importieren Sie die Datei nach wie vor per R-Befehl wie `read.csv()` (s. o.). $\square$
:::

::: {.content-visible when-format="html"}
![Daten importieren per Klick in Positron](img/import-data-positron.png){#fig-daten-positron width=50%}
:::
...
Alternativ können Sie Excel-Dateien *ohne* selbst geschriebenen R-Code importieren, per Klick-Import-Assistent -- in RStudio wie oben beschrieben, seit Version 2026.09 auch in Positron über `Import Data` (s. @sec-import-mariokart).
