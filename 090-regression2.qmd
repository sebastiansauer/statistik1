...
Die Autoren berichten über hohe Modellgüte ($r$) zwischen den tatsächlichen persönlichen Attributen 
...
also ähnlich zu dem in diesem Kapitel vorgestellten Methoden.

Neben der analytischen Stärke der Regressionsanalyse zeigt das Beispiel auch, 
wie gläsern Konsument:innen im Internet sind. $\square$
:::
...
(Im Datensatz ist die Temperatur ist in Grad Celcius angegeben; der Niederschlag  (`precip`) in mm Niederschlag pro Quadratmeter.)
...
Zur Erinnerung: Der Referenzwert bzw. der Wert der Referenzgruppe entspricht dem Y-Wert bei x=0 im Regressionsmodell.
...
Da eine binäre Variable wie eine metrische , 
...
Los R, rechnen!$ \square$
:::

>    [🤖]{.content-visible when-format="html"}[\emoji{robot}]{.content-visible when-format="pdf"} Endlich geht's weiter! Ergebnisse in @tbl-lm_wetter-month! 
...
Möchten Sie die herausgefilterten Faktorstufen "löschen", so können Sie einfach die Faktorvariable neu berechnen (mit `factor`).
...
Im zweiten Modell (`lm2`) kam `species` als zweiter Prädiktor 
...
d_sim %>%
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Oh nein, keine Korrelation!") +
