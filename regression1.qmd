...
Auf dieser Basis antworten Sie Toni: "Bei 42 Stunden Lernzeit solltest du so 40% Punkte bekommen. Könnte mit dem Bestehen eng werden."
...
noten2 %>% 
  ...
  annotate("label", y = mean(noten2$y), x = -Inf, 
           label = paste0("MW: ", round(mean(noten2$y))), hjust = "left")   +
  annotate("point", x = 42, y = predict(lm1, newdata = data.frame(x=42)), color = "red",
           alpha = .7, size = 4) +
...
Lies: "Laut meiner Modell ist $\hat{y}$ irgendeine Funktion von $y$".
...
```{r}
verhaeltnis_fehler_gerade_zu_punkt <- mae(lm1) / mae(lm0)
verhaeltnis_fehler_gerade_zu_punkt
```



Ah! Das Geradenmodell ist viel besser:
Von `lm0` zu `lm1` haben die mittlere (Absolut-)Länge des Fehlerbalkens auf `r round(verhaeltnis_fehler_gerade_zu_punkt, 2)*100` Prozent verbessert.
Nicht schlecht!
...
```{r}
verhaeltnis_fehler_gerade_zu_punkt <- mse(lm1)/mse(lm0)
verhaeltnis_fehler_gerade_zu_punkt
```
...
Es gibt verschiedene Algorithmen, um dies zu berechnen. 
...
Anders gesagt, wir haben uns um $1 - `r round(verhaeltnis_fehler_gerade_zu_punkt, 2)`$ verbessert:
...
:::{#def-r2}
### R-Quadrat
R-Quadrat eines Modells $m$ ist definiert als die Verringerung der Streuung, wenn man das Nullmodell $m_0$ mit dem Modell $m$ vergleicht: $R^2 =1-  \frac{\text{MSE}_{m}}{\text{MSE}_{m0}}$. R-Quadrat ist ein Maß der *Modellgüte*: Je größer, desto besser die Vorhersage. 
...
Im gegenteiligen Extremfall von $R^2=0$ ist die Vorhersage genauso gut wie wenn man für Y-Wert $\bar{y}$ vorhersagen würde. 
...
Die Residuen bestimmen die Modellgüte. Verschiedenen Koeffizienten stehen hier zur Verfügung: R-Quadrat, MSE, RMSE, MAE, ...
...
Man kann nur *deskriptiv* argumentieren.
...
"Im Modell `lm1` beträgt der Regressionskoeffizient `b1` $`r lm1_b1`$. Zwei Studenti, deren Lernzeit sich um eine Stunde unterscheidet, unterscheiden sich als *laut Modell* um den Wert von b1."
