...
Der Durchschnitt (das arithmetische Mittel, $\varnothing$) beträgt: 2.
:::
...
Bezeichnen wir die Abweichung - auch als "Fehler", "Rest" oder "Residuum" bezeichnet - der $i$-ten Person mit $e_i$ (*e* wie engl. *error*, "Fehler) und die $i$-te Note mit $x_i$, so können wir festhalten:
...
Mit $\hat{y}$ ist die vorhergesagte bzw. die zu erklärende Variable^[AV, Output-Variable, Zielvariable] gemeint.
Das "Dach"  über dem $y$ bedeutet "vorhergesagter Y-Wert" oder "Y-Wert laut dem Modell".
Der tatsächliche, beobachtete Wert $y$ setzt sich zusammen aus dem Modellwert (m) plus einem Fehler (e wie error):

$$y = m + e$$

Anstelle von $m$ schreibt man auch $\hat{y}$ ("y-Dach"). 
...
Die Zielvariable $Y$ wird also durch ihren eigenen Mittelwert erklärt, außer gehen wir von einem Fehler $e$ in unseren Modellvorhersagen aus. Nobody is perfect.
...
d_anim_decile <- 
  ...
  mutate(quart = cut(total_pr,
                     breaks = quantile(mariokart$total_pr, probs = k * .1),
                     include.lowest = TRUE, 
                     labels = 1:10)) |> 
  group_by(quart) |> 
...
anim_save("img/p_quartiles_anim.gif", p_quantiles_anim, renderer = gifski_renderer())
```
...
#| eval: false
k <- 1:100
breaks_percentiles <- quantile(mariokart$total_pr, probs = k * .01) 
...
Berechnen wir Lagemaße für den Mariokart-Datensatz, s. @lst-mario-lage
