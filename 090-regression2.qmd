...
Die Autoren berichten über eine hohe Modellgüte ($r$) zwischen den tatsächlichen persönlichen Attributen 
...
2. *Bereichschätzungen* Cleverer als Punktschätzungen sind Bereichsschätzungen (Intervallschätzungen): Hier wird ein Bereich plausibler Werte für den entsprechenden Wert angegeben. 
Der "Bereich plausibler Werte" wird auch als Konfidenzintervall (engl. confidence interval, CI) bezeichnet. 
Entsprechend gibt `CI_low` die Untergrenze des Bereichs plausibler Werte und `CI_high` die Obergrenze aus. 
So können wir ablesen, dass das Regressionsgewicht von `year` irgendwo zwischen praktisch Null (0.009) 
