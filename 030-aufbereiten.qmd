...
das hat irgendwie Eindruck geschunden …
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/030-aufbereiten.html) (oder als [PDF](slides/030-aufbereiten.pdf)).
:::
...
Die Spalte für den Verkaufspreis heißt offenbar `total_pr` (s. Datensatz `mariokart`).
...
Ah, die neuen Spiele sind teurer, wer hätt's gedacht!
...
Die Pfeife `|>` ist seit R 4.1 fest in R eingebaut (die "native pipe").^[Es gibt noch eine ältere Pfeife, `|>`, aus dem Paket `magrittr`,
welches vom Paket `tidyverse` geladen wird. Beide Pfeifen funktionieren praktisch gleich;
in diesem Buch verwenden wir durchgehend die eingebaute Pfeife `|>`.]
:::

Vor R 4.1 gab es nur die Pfeife aus dem `tidyverse` (genauer: aus dem Paket `magrittr`), geschrieben als `|>`.
Mittlerweile (seit R 4.1) ist auch im Standard-R eine Pfeife eingebaut, geschrieben als `|>`.
Die eingebaute Pfeife funktioniert praktisch gleich zur älteren Pfeife `|>`,
hat aber den Vorteil, dass Sie kein zusätzliches Paket dafür benötigen.
Deswegen verwenden wir in diesem Buch durchgehend die eingebaute Pfeife `|>`. Unter *Tools > Global Options …* können Sie einstellen,
...
So eine Pfeifen-Befehlssequenz ist ein wie ein Fließband,
...
![Studie COVIDiSTRESS [@lieberoth2022]](img/Covidistress1.jpg){width=50%}
...
Kurz, Sie brauchen die Hilfe eines kundigen Menschen?
