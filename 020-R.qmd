...
RStuido Cloud^[oder "Posit Cloud"] ist ein Webdienst von Posit/RStudio (zum Teil kostenlos), also *RStudio online*:
...
Ein Nachteil ist, dass es etwas langsamer ist und nur für ein gewisses Zeitvolumen kostenlos.
...
Namen des Pakets in der Paketliste (Reiter *Packages*) sehen, s. Abbildung @fig-install-packages.
...
:::{#exr-rquiz}
Ihre R-Muskeln sind gestählt? 💪 Oder doch noch nicht so ganz ausdefiniert? 😤 Macht nichts! Trainieren Sie sich mit [diesem Quiz](https://datenwerk.netlify.app/posts/r-quiz/r-quiz)! $\square$
:::
...
:::{#exr-texteditor}
### CSV-Datei von innen
🏋️‍♀️ Öffnen Sie mal eine CSV-Datei mit einem *Texteditor* (nicht mit Word und auch nicht mit Excel). Schauen Sie sich gut an, was Sie dort sehen und erklären Sie die Datenstruktur. $\square$
:::
...
Sie benötigen in diesem Kapitel den Datensatz `mariokart`, der entweder über [diese Internetadresse](https://vincentarelbundock.github.io/Rdatasets/csv/openintro/mariokart.csv) oder über R-Paket `openintro` importiert werden kann:
...
Natürlich gibt es viele Online-Kurse zu R, die aber teilweise kostenplichtig sind. Ein Beispiel ist der Kurs [Getting Started with Rstudio](https://www.coursera.org/projects/getting-started-rstudiol) (Kursdauer: 1 Stunde).
