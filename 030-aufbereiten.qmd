...
Denkt man an Data Science, stellt man sich coole Leute vor (in San Francisco oder Berlin), 
...
verbringen Data Scientisten "80%" ihrer Zeit mit dem *Aufbereiten* von Daten [@bowne-anderson_what_2018].
...
Flingt fast zu schön, um wahr zu sein (s. @fig-that-would-be-great).
...
die das Datenjudo in eine Handvoll einfacher Verben runterbrechen. 
...
dann funktionieren diese Befehle nicht.$\square$
:::
...
Was sind denn eigentlich die höchsten Preise, für die das Spiel *Mariokart* über den Online-Ladentisch geht?
Die Spalte des Verkaufspreis heißt offenbar `total_pr` (s. Datensatz `mariokart`).
...
Hm. Was ist mit den Auktionen, die *entweder* über (mind.) ein Photo verfügen *oder auch* neu sind, oder beides (Filtern mit dem logischen ODER)?
...
Viel besser wäre es doch, die Spalte `total_pr` zu einer Zahl zusammenzufassen,
das ist doch viel handlicher.
Kurz entschlossen fassen Sie die Spalte `total_pr`, den Verkaufspreis, 
zum Mittelwert zusammen, s. @fig-summarise.
...
also eine *Spalte* zu einer (einzelnen) *Zahl* zusammen, s. @eq-desk-summ.
...
p_d_g1 <- 
  p_d_g1 %>% 
  table_cell_bg(column = 3, fill = "lightblue", row = 2:tab_nrow(p_d_g1))
...
![Studie COVIDiSTTRESS [@lieberoth_covidistress_2020]](img/Covidistress1.jpg){width=50%}
...
[Sie sollten Ihren Hilfeschrei so artikulieren](https://data-se.netlify.app/2022/01/31/erbie-einfache-reproduzierbare-beispiele-ihres-problems-mit-r-syntax/), dass er nicht nur gehört, sondern auch verstanden wird und einen anderen Menschen veranlasst und ermöglicht Ihnen zu helfen.
