...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/010-rahmen.html) (oder als [PDF](slides/010-rahmen.pdf)).
:::
...
#| echo: false
#| label: fig-groesse
#| fig-cap: "Wenig Variation in der Körpergröße bei den Basketballern. Alles lange Kerle. Viel Variation bei den Schachspielern: Manche sind klein, andere groß. Die vertikalen, vom Mittelwert (MW) abgehenden Balken zeigen die Abweichungen der jeweiligen Körpergröße zum Mittelwert. Hinweis: Die Y-Achse startet nicht bei Null."
# dev: "ragg_png"
#| out-width: "75%"
...
groesse |> 
  ...
  scale_color_manual(values = c(beta0col, beta1col)) +
...
wie viel Ihre Nebensitzerinnen und Nebensitzer im Hörsaal so lernen.
...
::: {.content-visible when-format="html"}
![Schätzgenauigkeit als Funktion der Stichprobengröße: Die vertikale Linie zeigt den wahren Mittelwert. Kleinere Stichproben-Mittelwerte schwanken (variieren) mehr um den Mittelwert herum als größere Stichproben.](img/Estimate.gif){#fig-sample-estimate}
...
Je nachdem, über welches Skalenniveau eine Variable verfügt,
...
Die Ordinalskala erlaubt also, Objekte zu ordnen (hinsichtlich eines Merkmals).
...
- *stetige* Variablen, das sind Variablen, bei denen man zwischen zwei Ausprägungen immer noch eine weitere quetschen kann. So gibt es einen Wert für die Körpergröße zwischen 1.60&#8239;m und 1.61&#8239;m. Und einen Wert zwischen 1.601&#8239;m und 1.602&#8239;m, etc.
...
Ihr Kommilitone Paul behauptet:

> „Eigentlich gibt es doch nur *zwei* Ziele der Datenanalyse: beschreiben und vorhersagen. 'Erklären' ist gar kein eigenständiges drittes Ziel – wenn ich weiß, wie X mit Y zusammenhängt, kann ich Y doch direkt vorhersagen. 'Erklären' ist also nur ein anderes Wort für 'gut vorhersagen können'.“

Entwickeln Sie ein Gegenargument: Zeigen Sie an einem eigenen Beispiel, warum sich *Erklären* von *Vorhersagen* unterscheidet und daher ein eigenständiges drittes Ziel der Statistik ist.

### Lösung

Pauls Argument übersieht einen entscheidenden Unterschied: *Vorhersagen* braucht nur einen stabilen statistischen Zusammenhang zwischen X und Y – die Frage "warum" bleibt dabei unbeantwortet. *Erklären* dagegen fragt nach dem *kausalen* Mechanismus: Bewirkt X tatsächlich Y, und was passiert mit Y, wenn ich in X eingreife (interveniere)?

Ein klassisches Gegenbeispiel: Die Anzahl der Storchenpaare in einer Region und die Geburtenrate korrelieren (historisch belegt). Auf Basis dieses Zusammenhangs ließe sich die Geburtenrate sogar leidlich *vorhersagen*, wenn man die Storchenpopulation kennt. Trotzdem würde niemand daraus schließen, dass Störche Babys bringen – der Zusammenhang *erklärt* nichts, er beruht auf einer Drittvariablen (z.$\,$B. dem Urbanisierungsgrad einer Region, der sowohl Storchenlebensräume als auch Geburtenraten beeinflusst).

Ähnlich beim Beispiel aus @exm-zielarten: Aus der Lernzeit vieler Studierender lässt sich ein Vorhersagemodell für die Klausurnote bauen ("Wer 100 Stunden lernt, bekommt meist eine 2,3"), ohne dass klar ist, *ob* und *wie stark* das Lernen die Note *verursacht* – vielleicht lernen ohnehin nur die fleißigen, begabten Studierenden lange, und die Begabung, nicht die Lernzeit, erklärt die gute Note. Um zu erklären, braucht man zusätzliche Informationen (idealerweise ein Experiment mit zufälliger Zuteilung der Lernzeit), um eine reine Vorhersage von einer kausalen Erklärung zu unterscheiden.

Kurz: Ein Modell kann exzellent vorhersagen, ohne etwas zu erklären (Black-Box-Modelle wie neuronale Netze sind das Paradebeispiel), und ein Modell kann etwas erklären, ohne der beste Prädiktor zu sein. Beschreiben, Vorhersagen und Erklären sind daher tatsächlich drei unterscheidbare Ziele – nicht nur zwei. $\square$
:::
::::
...
Eine Kollegin verteidigt das File-Drawer-Problem:

> „Das ist doch keine *Lüge*! Beim File-Drawer-Problem werden ja keine Daten gefälscht – die 19 unspektakulären Variablen wurden schließlich wirklich gemessen, nur eben nicht berichtet. Man kann nicht jede Nebensache publizieren. Solange ich nichts erfinde, sondern nur auswähle, ist das doch redlich.“

Entwickeln Sie ein Gegenargument: Zeigen Sie, warum das Verschweigen unliebsamer Ergebnisse trotzdem eine Form des "Lügens mit Statistik" ist, obwohl keine einzige Zahl erfunden wurde.

### Lösung

Das Argument der Kollegin setzt "Lügen" mit "Daten erfinden" gleich. Aber Täuschung entsteht nicht nur durch falsche Zahlen, sondern auch durch die *selektive Präsentation wahrer* Zahlen, wenn dadurch ein falscher Eindruck von der Befundlage entsteht. Jede einzelne berichtete Zahl kann korrekt sein und die Gesamtdarstellung trotzdem irreführend – das ist der Kern von "Lügen mit Statistik".

Konkret: Testet man 20 Variablen jeweils auf dem üblichen Signifikanzniveau $\alpha = .05$, so ist rein durch Zufall (selbst wenn *keine* der Variablen tatsächlich einen Effekt hat) mit ungefähr $1-(1-.05)^{20} \approx 64\,\%$ Wahrscheinlichkeit mindestens ein "signifikantes" Ergebnis zu erwarten. Ein einzelner "Treffer" unter 20 Variablen ist also genau das, was man auch bei reinem Rauschen erwarten würde – kein Beleg für einen echten Effekt.

Wird nun nur diese eine Variable berichtet und werden die 19 Nullbefunde in der Schublade verschwiegen, entsteht beim Publikum der Eindruck eines soliden, replizierbaren Effekts, obwohl die Befundlage insgesamt für einen Zufallstreffer spricht. Die Leserschaft kann die tatsächliche Unsicherheit nicht mehr einschätzen, weil ihr die Information über die vielen erfolglosen Tests vorenthalten wird. Genau das ist die Definition von Publikationsbias: keine gefälschten Daten, aber eine verzerrte, irreführende Gesamtdarstellung – und damit eine Form des "Lügens mit Statistik", auch ohne eine einzige falsche Zahl. $\square$
:::
::::


::::{#exr-skalenniveau-argument}
:::{.panel-tabset}
### Aufgabe

Ein Kommilitone behauptet:

> „Temperatur in Grad Celsius ist verhältnisskaliert. Zehn Grad ist doch das Doppelte von fünf Grad – zehn ist ja auch das Doppelte von fünf. Also ist es an einem 10°C-Tag doppelt so warm wie an einem 5°C-Tag.“

Entwickeln Sie ein Gegenargument: Zeigen Sie, warum Celsius *nicht* verhältnisskaliert, sondern nur intervallskaliert ist – und warum die Aussage "10°C ist doppelt so warm wie 5°C" deshalb nicht sinnvoll ist.

### Lösung

Verhältnisaussagen ("doppelt so groß/warm/schwer") sind nur dann sinnvoll, wenn die Skala einen *echten, nicht-willkürlichen Nullpunkt* hat – einen Punkt, an dem die gemessene Eigenschaft tatsächlich *nicht vorhanden* ist. Bei Celsius ist $0\,°C$ aber willkürlich als Gefrierpunkt von Wasser festgelegt; es bedeutet nicht "keine Temperatur", also keine thermische Energie. Damit ist Celsius nur intervallskaliert: Abstände (Differenzen) sind interpretierbar, Verhältnisse nicht.

Der Fehler im Argument lässt sich leicht entlarven, indem man die Skala wechselt. Eine sinnvolle Verhältnisaussage muss unabhängig von der gewählten Maßeinheit gelten – genau das fordert die Definition des Skalenniveaus. Rechnet man $5\,°C$ und $10\,°C$ in Fahrenheit um, ergibt sich $41\,°F$ und $50\,°F$; das Verhältnis ist $50/41 \approx 1{,}22$, nicht $2$. Rechnet man stattdessen in Kelvin um (der einzigen der drei Skalen mit einem echten Nullpunkt, dem absoluten Temperaturnullpunkt), ergibt sich $278{,}15\,K$ und $283{,}15\,K$ – ein Verhältnis von $\approx 1{,}018$, also praktisch gar keine Verdopplung.

Dass "10 ist das Doppelte von 5" bei Celsius korrekt aussieht, bei Fahrenheit und Kelvin aber nicht, zeigt: Die vermeintliche Verdopplung ist ein Zufallsprodukt der Zahlenwerte auf einer willkürlich verschobenen Skala, keine physikalische Tatsache. Erlaubte Transformationen einer Intervallskala sind affine Transformationen ($y = a + b\cdot x$, mit $b>0$) – solche Transformationen erhalten Differenzen, aber nicht Verhältnisse. Nur bei einer Verhältnisskala (erlaubte Transformation: $y = b\cdot x$, also reine Skalierung ohne Verschiebung des Nullpunkts, wie bei Kelvin) bleiben Verhältnisaussagen über alle zulässigen Maßeinheiten hinweg stabil. Da "doppelt so warm" bei Celsius nicht einmal die Umrechnung in eine andere zulässige Temperaturskala übersteht, ist die Aussage kein sinnvolles Verhältnis, sondern ein Artefakt der Skala. $\square$
:::
::::
