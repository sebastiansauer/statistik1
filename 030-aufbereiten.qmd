...
Ein bisschen abstrakter gesprochen, fasst `summarise` 
 eine *Spalte* zu einer (einzelnen) *Zahl* zusammen, s. @fig-desk-summ.
...
Bei Auktionen mit Foto wird im Schnitt ein höhrer Preis erzielt als ohne Foto. $\square$
...
Die mittleren Versankosten bewegen sich also zwichen 2.7 Dollar und 3.6 Dollar, je nach Anzahl der beigelegten Lenkräder. $\square$
:::
...
Er ist einer der Hauptautoren von  beliebten R-Paketen wie `dplyr` und `ggplot2`. @wickham_r_2018 Kap. 5 behandelt (etwas ausführlicher) die Themen dieses Kapitels.
