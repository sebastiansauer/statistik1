...
Verteilungen unterscheiden sich z.B. in ihrem "typischen" oder "mittleren" Wert^[vgl. @sec-streuung] und/oder in ihrer Streuung, s.@fig-vert-dif.

Im Histogramm (oder Dichtediagramm) erkennt man den mittleren Wert an der Lage des "Berggipfel"; so sind die "Berggipfel" in @fig-vert-dif unterschiedlich weit links oder rechts auf der X-Achse positioniert.
Man erkennt im Histogramm die Streuung an der "Breite des Berges", s. @fig-vert-dif.


```{r}
#| echo: false
#| fig-cap: Unterschiede in Mittelwert und Streuung zwischen Verteilungen
#| layout-ncol: 2
#| label: fig-vert-dif
#| fig-subcap: 
#|   - Unterschiedliche Mittelwerte, etwa gleiche Streuung
#|   - Etwa gleiche Mittelwerte, unterschiedliche Streuungen
...
d %>% 
  ggplot() +
  geom_density(aes(x = x1), fill = "red", alpha = .3) + 
  geom_density(aes(x = x2), fill = "green", alpha = .3) + 
  geom_density(aes(x = x3), fill = "blue", alpha = .3) + 
  theme_minimal() +
  labs(x = "Variable",
...
d %>% 
  ggplot() +
  geom_density(aes(x = x1), fill = "red", alpha = .3) + 
  geom_density(aes(x = x4), fill = "green", alpha = .3) + 
  geom_density(aes(x = x5), fill = "blue", alpha = .3) + 
  theme_minimal() +
  labs(x = "Variable",
       y = "relative Häufigkeit")
...
(Diagramme von) Verteilungen können sich auch in ihrer *Schiefe* unterscheiden: 
Verteilungen können symmetrisch oder schief (nicht symmetrisch) sein, s. @fig-symm-schief.
...
Eine Normalverteilung lässt sich exakt beschreiben anhand zweier Parameter: 
ihres zentralen Werts (Mittelwerts), $\mu$ und ihrer Streuung (Standardabweichung), $\sigma$,
s. @fig-nv-div.
...
#| echo: false
#| label: fig-nv-div 
#| fig-cap: Verschiedene Normalverteilungen, die sich in Mittelwert und Streuung unterscheiden
...
ggplot(tibble(x = -5:5)) +
  ...
  stat_function(fun = dnorm, args = list(mean = -2, sd = 1), color = "green") +
  stat_function(fun = dnorm, args = list(mean = 1, sd = 2), color = "blue") +
  stat_function(fun = dnorm, args = list(mean = -1, sd = 2), color = "orange") +
  theme_minimal() +
  labs(x = "Variable",
       y = "relative Häufigkeit")
```
...
Kennt man diese beiden Parameter $\mu$ und $\sigma$, 
so kann man einfach angeben, welcher Anteil der Fläche sich in einem bestimmten Bereich befindet, s. @fig-norm-perc.
