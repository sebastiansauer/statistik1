...
Prof. Weiss-Ois hat eine Erfindung gemacht, eine Schlankheitspille [💊]{.content-visible when-format="html"}  ...
...
:::{#def-range}
Ein einfaches Streuungsmaß ist der *Range* $R$, definiert als Abstand von größtem und kleinsten Wert eines Merkmals $X$: $R = X_{max} - X_{min}. $\square$
:::

:::{#exm-range}
Angenommen, wir haben einen Datensatz zum Merkmal "Alter" mit den Werte 1, 23, 42, 100. Dann beträgt der Range: $R = 100 - 1 = 99$. Das bedeutet, dass die Werte des Merkmals über 99 Einheiten (Jahre in diesem Fall) verteilt sind. $\square$
:::
