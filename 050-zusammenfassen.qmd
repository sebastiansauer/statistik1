...
![Der Mittelwert als "typisches Element" einer Zahlenreihe](img/mw-modell-crop2.png){#fig-mw2 width="75%"}
...
Der Nutzen des Mittelwerts liegt darin, dass er einen Vektor (eine "Zahlenreihe") 
zu einen "typischen Vertreter" zusammenfasst.
...
![Der Mittelwert der Zahlenreihe $(1,2,6)$ als ausbalancierte Wippe mit Mittelwert 3 [@maphry_seesaw_2009]](img/1280px-Seesaw_with_mean.svg.png){#fig-wippe width="70%"}
...
:::{#def-mw}
### Mittelwert
Der Mittelwert (MW, mean) von $X$ (präziser: das arithmetische Mittel des Merkmals $X$) 
ist definiert als die Summe der Elemente von $X$ geteilt durch deren Anzahl, $n$. 
...
### Die Summe der Abweichungen ist Null
...
Vier Studentinnen (Anna, Berta, Carla, Dora, Emma) tauschen sich über ihr Einkommen (1, 2, 3, 4, 100) aus, s. @fig-md1, links.
Es handelt sich um eine schiefe Verteilung.
Wir könnten jetzt behaupten, dass Carla das typische Einkommen (für diese Datenreihe) aufweist (3),
da es genauso viele Studentinnen gibt, die mehr verdienen, wie solche, die weniger verdienen. $\square$
:::

```{r}
#| label: fig-md1
#| fig-cap: Das Einkommen einiger Studentinnen (1,2,3,4, 100) sowie der Mittelwert (MW=22) und Median (Med=3) ihres Einkommens
#| echo: false
#| out-width: 75%
#| layout-ncol: 1
#| fig-asp: 0.618
...
d %>% 
  ...
  annotate("label", x = 100, y = +2, label = "Emma") +
  annotate("label", x = 0, y = 2, label = "Anna\nBerta\nCarla\nDora", hjust = 0, vjust = "top") +
  labs(caption = "MW: Mittelwert",
       y = "",
       x = "Einkommen")
```
...
:::{#exm-med2}
Für die Stichproben $(1,2,3)$ und $(1,2)$ gilt:
$\operatorname{Median}(1,2,3) = 2$ und 
$\operatorname{Median}(1,2) = \frac{1+2}{2} = 1.5$. $\square$
:::
...
@fig-nv-quants2 zeigt einige häufig verwendete Quantile für die IQ-Verteilung. $\square$
