...
Zuerst definieren wir den Pfad, wo wir die Daten finden, s. @lst-mariokart-path-pdf.


```{r mariokart-path-pdf}
#| lst-label: lst-mariokart-path-pdf
#| lst-cap: Pfad zu den Mariokart-Daten
...
s. @fig-dino1.^[Quelle: <https://towardsdatascience.com/how-to-turn-a-dinosaur-dataset-into-a-circle-dataset-with-the-same-statistics-64136c2e2ca0>]
...
In die gleiche Bresche schlägt "Anscombes Quartett" [@Anscombe1973], s. @fig-dino2:
Es zeigt vier Datensätze, in denen die zentralen Statistiken fast identisch sind,    
also Mittelwerte, Streuungen, Korrelationen.
Aber die Streudiagramme sind grundverschieden.^[Quelle: <https://en.wikipedia.org/wiki/Anscombe%27s_quartet>]
...
![Anscombes Quartet: Gleiche statistischen Kennwerte in vier Datensätzen](img/anscombe.png){#fig-dino2}
...
@fig-many-dims zeigt ein Bild mit mehreren (5) Variablen, die jeweils einer "Dimension" entsprechen.
...
ist, die "Flatlands" zu verlassen, also von 2D auf 3D und/oder auf interaktive Diagramme zu wechseln, [s. hier](https://plotly.com/r/3d-surface-plots/), 
s. @fig-3d-earth.^[Quelle: Paul Buffa, d3js, <https://observablehq.com/@pstuffa/faux-3d-globe>]
...
@tbl-nom-plots zeigt eine - sehr kurze Nomenklatur - an Datendigrammen.^[Weitere Nomenklaturen sind möglich, aber wir halten hier die Sache einfach. Wer an Vertiefung interessiert ist, findet bei data-to-vis einen Überblick über verschiedene Typen an Diagrammen, sogar in Form einer systematischen Nomenklatur: <https://www.data-to-viz.com/>.]
...
::: {.content-visible when-format="html"}
Zuerst importieren wir die Daten, s.  @lst-mariokart und @lst-mariokart-path.
:::

::: {.content-visible when-format="pdf"}
Zuerst importieren wir die Daten, s. @lst-mariokart und @lst-mariokart-path-pdf.
:::


```{r}
#| lst-cap: "Mariokart-Daten importieren von einer Webseite"
...
In diesem Paket "wohnen" die Befehle, die wir zum Erstellen der Datendiagramme nutzen werden. @lst-de1 zeigt die Syntax, um ein Balkendiagramm zu erstellen.^[Auf der Hilfeseite der Funktion finden Sie weitere Details zur Funktion.]


```{r}
#| lst-label: lst-dataexplorer
#| lst-cap: "Wir starten das R-Paket DataExplorer"
#| eval: false
library(DataExplorer)
```
...
Die Syntax ist in @lst-de1 abgedruckt^[Zur Erinnerung: ` %>% ` nennt man die "Pfeife und lässt sich als "und dann" übersetzen, vgl. @sec-pipe.] . Übersetzen wir die Syntax ins Deutsche:
...
Bei einer quantitativen Variablen mit vielen Ausprägungen wäre ein Balkendiagramm nicht so aussagekräftig, s. @fig-balken-hist (links). Es gibt einfach zu viele Ausprägungen.
...
:::{#def-histogramm}
#### Histogramm
Ein Histogramm ist ein Diagramm zur Darstellung der Häufigkeitsverteilung einer quantitativen Variablen. Die Daten werden in Gruppen (Klassen) eingeteilt, die dann durch einen Balken (pro Klasse) dargestellt sind.
...
Es gibt keine klare Regel, in wie viele Balken ein Histogramm gegliedert sein sollte.
...
Kennt man diese beiden Parameter, so kann man einfach angeben, welcher Anteil der Fläche sich in einem bestimmten Bereich befindet, s. @fig-norm-perc.^[Quelle: Ainali - Own work, CC BY-SA 3.0, <https://commons.wikimedia.org/w/index.php?curid=3141713>]
...
![Die Flächeninhalte (Wahrscheinlichkeitsmasse) einer Normalverteilung in Abhängigkeit der SD-Einheiten](img/Standard_deviation_diagram_micro.svg.png){#fig-norm-perc}
...
Man sieht in @fig-cors und @fig-cors2, dass ein *negativer* Korrelationskoeffizient mit einer *absinkenden* Trendgerade ^[synonym: Regressionsgerade] (blaue Linie) einhergeht.
...
Das rechte Teildiagramm zeigt die gleichen Verteilungen, aber mit einer vereinfachten, groberen Darstellungsfrom, den *Boxplot*.^[Übrigens: Freunde lassen Freunde nicht Balkendiagramme verwenden, um Mittelwerte darzustellen: <https://github.com/cxli233/FriendsDontLetFriends#1-friends-dont-let-friends-make-bar-plots-for-means-separation>.]
...
Was ein "deutlicher"^["substanzieller", "bedeutsamer", "relevanter" oder "(inhaltlich) signifikanter"] Zusammenhang ist, ist keine statistische, sondern inhaltliche Frage, die man mit Sachverstand zum Forschungsgegenstand beantworten muss.
...
Das Diagramm und weitere finden sich in ähnlicher Form im [Wall Street Journal](http://graphics.wsj.com/infectious-diseases-and-vaccines/).


![Häufigkeit von Masern und Impfung in den USA, Lizenz: MIT](img/vaccine.jpg){#fig-vaccine width=75%}

[Quellcode](https://github.com/blmoore/blogR/blob/master/R/measles_incidence_heatmap.R)^[Datenquelle: <https://www.tycho.pitt.edu>]
...
Weniger spektakulär als @fig-3d-earth, aber näher an der Datenanalyse ist @fig-mario-3d.
...
Natürlich sind der Fantasie keine Grenzen beim Visualisieren gesetzt, so ist etwa [diese Animationen](https://www.tylermw.com/wp-content/uploads/2019/06/featuredmeasles.mp4) ziemlich atemberaubend.^[<https://www.tylermw.com/wp-content/uploads/2019/06/featuredmeasles.mp4>]
...
Manchmal auch wichtige Informationen. [Dieses Beispiel](https://www.autodesk.com/research/publications/same-stats-different-graphs) zeigt, wie etwa Histogramme wichtige Informationen unter den Tisch fallen lassen.^[<https://www.autodesk.com/research/publications/same-stats-different-graphs>]
...
Möchte man auf die Kennzahlen verzichten, so nutzt man den Schalter `results.subtitle = FALSE`.^[Weitere Hinweise finden sich [auf der Hilfeseite der Funktion: <https://indrajeetpatil.github.io/ggstatsplot/articles/web_only/gghistostats.html>].
...
Die Farbpalette von Okabe und Ito ist [vgl. @ichihara_color_2008] empfehlenswert, da sie auch bei Schwarz-Weiß-Druck und bei Sehschwächen die Farben noch recht gut unterscheiden lässt, s. @fig-okabe.
