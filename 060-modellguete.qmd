...
Außerdem gilbt: Sind Ihre Daten normalverteilt, dann sind die Abweichungen vom Mittelwert auch normalverteilt.
...
Links sind die *Abweichungsqudrate* dargestellt, rechts die Varianz als "*typisches Abweichungsquadrat*".
...
Als Formel ausgedrückt, lautet die Definition der Varianz^[sog. unkorrigierte Stichprobenvarianz] einer Stichprobe wie folgt, s. @eq-var.
...
:::{#def-var}
### Varianz
Die Varianz ($s^2, \sigma^2$) ist definiert als der Mittelwert der quadrierten Abweichungen (vom Mittelwert).$\square$
:::
...
gghistogram(mariokart2, x = "total_pr", 
            ...
            add.params = list(color = "blue", size = 3)) # Schnickschnack zur Verschönerung

gghistogram(mariokart2, x = "abw",
            add = "mean",  # Mittelwert wird hinzugefügt
            add.params = list(color = "blue", size = 3))
```
...
d %>% 
  ggplot() +
  geom_density(aes(x = groesse), color = "#E69F00FF") +
  geom_density(aes(x = groesse_zentriert), color = "#56B4E9FF") +
  theme_minimal() +
...
#| echo: false
#| label: fig-z-transf
#| fig-cap: Standardisierung von Abweichungswerten bzw. einer Verteilung
...
Wir fassen unsere Schritte unserer Umrechnung ("Transformation") zusammen:
