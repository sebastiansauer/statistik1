...
Also führt Fritz seine Kuh durch den Fluss, leider kam die Kuh nicht am anderen Ufer an, da fim Floß ersoffen, s. @fig-fluss-tief.
...
Ein Modell ohne Angaben der Modellgüte, d.h. der Präzision der Schätzung des Modellwerts, ist wenig nütze.
...
dass wir die Lenkräder-Anzahl da irgendwie nutzen zur Vorhersage könnten.
...
Der IQR beträgt dann: $IQR = Q_3-Q_1 = 1.75 \; m - 1.65\;  m = 0.10\; m$, d.h. 10 cm. $\square$
:::
...
Als Formel ausgedrückt lautet die Definition der Varianz einer Stichprobe der Größe $n$ wie folgt, s. @eq-var. (Hier geht es um die sog. unkorrigierte Stichprobenvarianz; um anhand einer Stichprobe die Varianz der zugehörigen Population zu schätzen, teilt man nicht durch $n$, sondern durch $n-1$.)
...
mae(lm1)  # Modellgüte bzw. Modellfehler
```
