...
mtcars %>% 
  filter(am = 0)  # den kürzsten Code, der Ihren Fehler entstehen lässt!
```
...
Andere, gebräuchliche Befehle, die CSV-Dateien importieren, heißen `read.csv()` (aus dem Standard-R, kein Extra-Paket nötig) und `read_csv()` (aus dem Paket `{tidyverse}`).
`data_read()` ist praktisch, da es viele verschiedenen Datenformate lesen kann und selbständig erkennt, um welches Datenformat (CSV, XLSX,...) es sich handelt.
