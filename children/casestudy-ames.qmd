...
Sie können auch über das Github-Repo `statistik1`, Ordner die Daten zugreifen:
...
#| message: false
#| eval: false
d_train_path <- "data/ames-kaggle/train.csv"
d_test_path <- "data/ames-kaggle/test.csv"
...
In diesem Beispiel gehen wir davon aus, dass die Dateien `train.csv` und `test.csv` in einem Unterordner namens `data/ames-kaggle` liegen.
