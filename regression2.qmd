...
mariokart_train <- 
  mariokart %>% 
  sample_frac(size = .7) %>% 
  select(-X, -id, -title)  # diese Variablen brauchen wir nicht zum Vorhersagen
```
...
Um die Vorhersage-Güte auszurechnen, nutzen wir die Funktionen des R-Paketes `yardstick`^[welches Sie vielleicht noch installieren müssen.]:

```{r}
library(yardstick)

mae(data = mariokart_test,
    truth = total_pr,  # echter Verkaufspreis
    estimate = lm_allin_predictions)  # Ihre Vorhersage
```

Ihr mittlerer Vorhersagefehler (MAE) liegt bei ca. 12 Euro. 
...
>   🧔‍♂️ Nicht berauschend, aber solide!



### Fazit

In diesem Kapitel haben Sie lineare Modelle gelernt, die über einfache Modelle der Art `y ~ x` hinausgehen. Dazu gehören multiple Modelle, das sind Modelle mit mehr als einer UV (Prädiktor) und auch Interaktionsmodelle.
